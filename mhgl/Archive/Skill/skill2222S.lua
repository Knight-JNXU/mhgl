...
function skill2222CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需熬寒诀修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill2222Use(Atk, Target, battle, skillLv)

	local consumeSp = 0.8 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展绕指柔", consumeSp);
	...
	if Target:IsBuffExist(1103) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了绕指柔");
		do return end
	end
	...
	if math.random(1, 100) > HitRate then
		...
		if Target:IsBuffExist(3093) then
			Atk:AddBuff(1103, skillLv, 0, 0, turn, 100);
		end
		do return end
	end
	...
	if result == 2 then
		...
		Target:AddBuff(1103, skillLv, 0, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
