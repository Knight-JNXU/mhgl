...
function skill9892Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展一剑西来", consumeSp);
	...
	while targetlist[j] ~= nil do
		...
		local FinalDam = 1.2 * ((Atk:GetPOW() - targetlist[j]:GetPOW() + (3.4 - 0.3 * count) * lv + nDamsBonus1 * lv + 250) * nDam - 250) * RateDam * randomDam;
		...
		if math.random(1,100) <= 10 and targetlist[j]:IsBuffExist(1440) ~= true and targetlist[j]:GetHP() ~= 0 then 
			targetlist[j]:AddBuff(1440, lv, 0, 0, 3, 100);
		end
