...
function skill6202CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill6202Use(Atk, Target, battle, skillLv)
	...
	if Atk:GetMob_id() == 32021 or Atk:GetMob_id() == 32022 then
		maxTargetCount = 5
	elseif Atk:GetMob_id() == 30727 then
		maxTargetCount = 4
	end
	...
	while targetlist[i] ~= nil do
		...
		if Atk:GetMob_id() == 30727 then
			hpChange = ((Atk:GetPOW() - targetlist[i]:GetPOW() + (3.4 - 0.3 * count) * Lv + nDamsBonus1 * Lv + 250) * nDam - 250);
		elseif Atk:GetMob_id() == 8032 then
			hpChange = ((Atk:GetPOW() - targetlist[i]:GetPOW() + (3.4 - 0.3 * count) * Lv + nDamsBonus1 * Lv + 250) * nDam - 250) + 3 * Lv;
		end
