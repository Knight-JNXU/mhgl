...
function skill0315CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill0315Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展地动山摇", consumeSp);
	...
	if lianji == 1 then
		...
		while targetlist[j] ~= nil do
			...
			while i < lianji do
				...
				if HitRate < 8 and targetlist[j]:IsBuffExist(1404) ~= true and targetlist[j]:GetHP() ~= 0 then
					targetlist[j]:AddBuff(1404, lv, effectHIT, effectEVA, 3, 100);
				end
