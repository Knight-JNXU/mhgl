...
function skill0075CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill0075Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 0.5 * skillLv + 10;
	
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展游魂丝", consumeSp);
	...
	if math.random(1, 100) <= BuffRate then
		Target:AddBuff(72, 0, 0, 0, turn, 100); 
	end
