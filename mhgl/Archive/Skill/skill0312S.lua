...
function skill0312CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill0312Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展风驰电掣", consumeSp);
	...
	if lianji == 1 then
		...
		while targetlist[j] ~= nil do
			...
			local effectDef = (1 - 0.05) * targetlist[j]:GetDEF();
			...
			if HitRate < 20  and targetlist[j]:IsBuffExist(1406) ~= true and targetlist[j]:GetHP() ~= 0 then 
				targetlist[j]:AddBuff(1406, lv, effectDef, 0, 3, 100);
			end
		...
		while targetlist[j] ~= nil do
			...
			while i < lianji do
				...
				if HitRate < 8  and targetlist[j]:IsBuffExist(1406) ~= true and targetlist[j]:GetHP() ~= 0 then 
					targetlist[j]:AddBuff(1406, lv, effectDef, 0, 3, 100);
				end
