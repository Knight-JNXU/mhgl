...
function skill4235CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4235Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：酒意不足\#G%d\#W，无法施展聚华凝神", consumeDp);
	...
	while targetlist[i] ~= nil do
		...
		local rec = 0.15 * targetlist[i]:GetMaxSP() + targetlist[i]:GetLevel() * (Atk:GetWC() / 100);
		if rec <= 1 then
			rec = 1;
		end
