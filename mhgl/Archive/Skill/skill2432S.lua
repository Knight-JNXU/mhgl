...
function skill2432CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill2432Use(Atk, Target, battle, skillLv)
	
	if Target:IsBuffExist(1804) then
	...
	local reAtkCount = 1 + math.floor((skillLv - 30) / 25);
	
	if reAtkCount < 1 then
		reAtkCount = 1;
	elseif reAtkCount > 4 then
		reAtkCount = 4;
	end
	...
	while targetlist[i] ~= nil do
		...
		targetlist[i]:AddBuff(1804, skillLv, reAtkCount, 0, turn, 100);
