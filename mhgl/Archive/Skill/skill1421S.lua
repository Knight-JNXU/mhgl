...
function skill1421CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill1421Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展疾光电影",consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		local hpChange = ((Atk:GetPOW() - targetlist[i]:GetPOW() + 0.25 * weaponDam) * 0.8 + 
						(1.7 - 0.1 * count) * skillLv + 0.1 * skillDam + 260)  * nDam - 250;
		if LuaI.GetGameTime(0) == 1 then
			hpChange = hpChange * 1.3;
		end
		if targetlist[i]:IsMob() then
			hpChange = hpChange * 1.1;
		end
		if hpChange <= 10 then
