...
function skill2443CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill2443Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 0.8 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展重云深锁", consumeSp);
	...
	if Target:IsBuffExist(1120) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了重云深锁");
		do return end
	end
