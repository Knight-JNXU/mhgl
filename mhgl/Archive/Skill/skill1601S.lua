...
function skill1601CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	...
	if Atk:GetHP() < reqHp then
		local Msg = "\#W生命不足\#G20\%\#W，无法施展踏月步";
		Atk:SkillFail(Msg);
		do return end
	elseif Atk:GetHP() < consumeHp then
		local Msg1 = string.format("\#W生命不足\#G%d\#W，无法施展踏月步", consumeHp);
		Atk:SkillFail(Msg1);
		do return end
	elseif Atk:GetSP() < consumeSp then
		local Msg2 = string.format("\#W内力不足\#G%d\#W，无法施展踏月步", consumeSp);
		Atk:SkillFail(Msg2);
		do return end
	elseif Atk:HasParty() then
		local Msg = "\#W组队状态下无法施展踏月步";
		Atk:SkillFail(Msg);
		do return end
...
function skill1601Use(Atk, Target, battle, skillLv)
