...
function skill5043CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill5043Use(Atk, Target, battle, skillLv)	
	local targetlist = {};	
	local Lv = Atk:GetLevel();
	local maxTargetCount = math.floor((Lv - 35) / 25) + 2;
	...
	if skill == 1 then
	    maxTargetCount = math.floor((Lv - 35) / 25) + 3;
	...
	while targetlist[i] ~= nil do
		...
		if not targetlist[i]:IsBuffExist(1410) then 
