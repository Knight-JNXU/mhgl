...
function skill2811CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill2811Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展拈花拂柳", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		local FinalDam = ((Atk:GetPOW() + weaponDam - targetlist[i]:GetPOW()) * 0.5 + (1.5 - 
						0.05 * count) * skillLv + 0.1 * skillDam + 260) * nDam - 250;
		if FinalDam <= 10 then
