...
function skill2643CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill2643Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 0.5 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展云游太虚", consumeSp);
	...
	if Target:IsBuffExist(1121) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了云游太虚");
		do return end
	end
	...
	if skillLv > Target:GetLevel() then
		HitRate = (66 + 2 * (skillLv - Target:GetLevel()) ^ 0.5) * nDam;
	else
		HitRate = (66 + 2.2 * (skillLv - Target:GetLevel())) * nDam;
	end
	...
	if math.random(1, 100) > HitRate then
		...
		if Target:IsBuffExist(3093) then
			Atk:AddBuff(1121, skillLv, 0, 0, turn, 100);
		end
		do return end
	end
	...
	if result == 2 then
		...
		Target:AddBuff(1121, skillLv, 0, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
