...
function skill0632CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill0632Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展晓风残月剑", consumeSp);
	...
	while targetlist[j] ~= nil do
		...
		if math.random(1,100) <= 10 and targetlist[j]:IsBuffExist(1442) ~= true and targetlist[j]:GetHP() ~= 0 then 
			targetlist[j]:AddBuff(1442, lv, 0, 0, 3, 100);
		end
