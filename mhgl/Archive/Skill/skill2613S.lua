...
function skill2613CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill2613Use(Atk, Target, battle, skillLv)

	local consumeSp = 0.8 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展惊神断魄", consumeSp);
	...
	if Target:IsBuffExist(1116) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了惊神断魄");
		do return end
	end
