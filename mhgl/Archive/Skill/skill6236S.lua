...
function skill6236CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill6236Use(Atk, Target, battle, skillLv)
	local Lv = Atk:GetLevel();
	local skill1 = Atk:GetSkillLv(5300);	
	local targetlist = {};
	local maxTargetCount = 8;	
	local bufflist = {};
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if CritRate <= 5 then											
				FinalDam = 1.5 * ((1.05 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * critDam;
				CritHit = 1;						
			else
				FinalDam = 1.5 * ((1.05 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam;
			end
