...
function skill6238CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill6238Use(Atk, Target, battle, skillLv)
	local Lv = Atk:GetLevel();
	local skill1 = Atk:GetSkillLv(5300);	
	local targetlist = {};
	local maxTargetCount = 7;	
	local bufflist = {};
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if result == 2 then
				...
				if math.random(1, 100) <= 20 and not targetlist[i]:IsBuffExist(1124) then
					targetlist[i]:AddBuff(1124, (Lv + 10), 0, 0, 3, 100);
				end
			end
