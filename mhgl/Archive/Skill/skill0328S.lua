...
function skill0328CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill0328Use(Atk, Target, battle, skillLv)
	--[[local lv = Atk:GetLevel();
	local consumeSp = 0.6 * skillLv + 10;
	
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法吸血", consumeSp);
	...
	if math.random(1,100) <= 5 then
		...
		rec = ((0.25 * (Atk:GetATK() - FinalDef) + 0.5 * skillLv) * 1.05 ^ wDam) * randomDam * critDam;
		CritHit = 1;
	else
		FinalDam = (Atk:GetATK() - FinalDef + Lv)  * randomDam;	
		rec = (0.25 * (Atk:GetATK() - FinalDef) + 0.5 * skillLv) * randomDam;
	end
