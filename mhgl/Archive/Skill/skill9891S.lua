...
function skill9891CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	
end

function skill9891Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展灵犀一指", consumeSp);
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if math.random(1,100) <= CritRate then
				FinalDam = ((1.05 * Atk:GetATK() - 0.8 * FinalDef + 250) * wDam - 250) * randomDam * critDam;
				CritHit = 1;
			else
				FinalDam = ((1.05 * Atk:GetATK() - 0.8 * FinalDef + 250) * wDam - 250) * randomDam;
			end
