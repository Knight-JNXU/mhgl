...
function skill9901CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill9901Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展龙翔凤舞", consumeSp);
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if math.random(1,100) <= CritRate then
				FinalDam = (((1.2 * Atk:GetATK() - 0.95 * FinalDef)*(1.3-0.3*count) + 250) * wDam - 250) * randomDam * critDam;
				CritHit = 1;
			else
				FinalDam = (((1.3 * Atk:GetATK() - FinalDef)*(1.3-0.3*count) + 250) * wDam - 250) * randomDam;
			end
			...
			if math.random(1,100) <= 10 and targetlist[i]:IsBuffExist(1401) ~= true and targetlist[i]:GetHP() ~= 0 then 
				targetlist[i]:AddBuff(1401, Lv, 0, 0, 3, 100);
			end			
