...
function skill6224CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill6224Use(Atk, Target, battle, skillLv)
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if targetlist[i]:GetDEF() <= 1.3 * targetlist[i]:GetPOW() then
				if math.random(1,100) <= CritRate then				 
					FinalDam = 0.65 * ((Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * critDam;
					CritHit = 1;
				else
					FinalDam = 0.65 * ((Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam;
				end
			else
				if math.random(1,100) <= CritRate then				 
					FinalDam = 0.65 * ((Atk:GetATK() - FinalPow + 250) * wDam - 250) * randomDam * critDam;
					CritHit = 1;
				else
					FinalDam = 0.65 * ((Atk:GetATK() - FinalPow + 250) * wDam - 250) * randomDam;
				end
			end
