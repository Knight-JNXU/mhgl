	...
	if Atk:GetTeam() == Def:GetTeam() then
		if Atk:HasBuff(1113) then
			...
			if Atk:GetATK() * 0.8 > Def:GetDEF() and num <= 5 then
				dam = ((Atk:GetATK() - Def:GetDEF() / 2 + 500) * wDam - 500) * math.random(900, 1100) / 1000;
			else
				dam = ((Atk:GetATK() - Def:GetDEF() + 500) * wDam - 500) * math.random(900, 1100) / 1000;
			end
		end			
	else
		local num = math.random(1, 100);
...
function playerExpGain(actor, killCount, npcLevel, npcExp, teamCount, mapLevel)--玩家获取经验设定（角色练功丸）
    ...
    --玩家获得经验	
	expGain = actor:GetOwner():GetAgeCheckState() * 0.5 * math.floor(npcExp * (4 * npcLevel + 10) / (4 * mapLevel + 14) * killCount * teamCountModify * lvDiffModify * DoubleModify2 * DoubleModify3 * DoubleModifyworld * JieriModify * BattleModify*DoubleModifyHFT*DoubleModify4 * 2);--玩家升级经验10倍
...
function partnerExpGain(actor, owner, killCount, npcLevel, npcExp, teamCount, mapLevel)--同伴获取经验设定（同伴练功丸）
	...
	if lvDiff_owner < 5 then
	    expGain = owner:GetOwner():GetAgeCheckState() * 0.5 * math.floor(1.5*npcExp * (4 * npcLevel + 10) / (4 * mapLevel + 14) * killCount * teamCountModify * lvDiffModify * DoubleModify2 * DoubleModify3 * DoubleModifyworld * BattleModify * JieriModify*DoubleModify4*DoubleModifyHFT * 2);--同伴升级经验20倍
...
function CalPlayerNextLvExp(Lv)--玩家升级所需经验公式
	...
	elseif  Lv >= 120 and Lv <= 130 then
		result = 116 * Lv - 12523.84 ; 		
	elseif  Lv >= 130 and Lv <= 140 then
		result = 126 * Lv - 12523.84 ; 		
...
function CalPartnerAttr(partner, needDecrease)--同伴战斗中属性
	...
	local atk = level*(partner:GetATK_native() + NativeUp)*7/2000+str*grow;
		
	local def = level *(partner:GetDEF_native() + NativeUp)/433+con* grow*4/3;
	
	local spd = agi * (partner:GetSPD_native() + NativeUp) / 1000 ;
	
	local power = (((partner:GetSP_native() + NativeUp)+1640)/100)*((grow +1)/75)*level+spr * 0.7 + str * 0.4 + sta * 0.3 + con * 0.2;
	...
	local maxHP = (partner:GetHP_native() + NativeUp) * level / 1000 + grow * sta * 6;
	...
	if Race == 1 then
		...
		atk = level*(partner:GetATK_native() + NativeUp)*7/2000+str*grow;
		
		def = level *(partner:GetDEF_native() + NativeUp)/433+con* grow*4/3 ;
	
		spd = 0.5 * agi * (partner:GetSPD_native() + NativeUp) / 1000 ;
		
		power = (((partner:GetSP_native() + NativeUp)+1640)/100)*((grow +1)/75)*level+spr * 0.7 + str * 0.4 + sta * 0.3 + con * 0.2;
		...
		maxHP = (partner:GetHP_native() + NativeUp) * level / 1000 + grow * sta * 6;
...
--[[batActor and batMob is pointing to same object
	...
	local atk = level*batMob:GetATK_native()*7/2000+str*grow;
		
	local def = level * batMob:GetDEF_native()/433+con* grow*4/3;
	
	local spd = agi * batMob:GetSPD_native() / 1000 ;
	
	local power = ((batMob:GetPOW_native()+1640)/100)*((grow +1)/75)*level+spr * 0.7 + str * 0.4 + sta * 0.3 + con * 0.2;
	...
	local maxHP = batMob:GetHP_native() * level / 1000 + grow * sta * 6;
...
--[[
 ...
 local eq_MaxHP = 0;
   local eq = pChar:GetBag():GetItem(120)  ;
    ...
    local atk = (str * pCharData:GetAtk_STR() + 30)/3 + 30+eq_MaxHP;
...
--[[
    ...
    local atk = (str * pCharData:GetAtk_STR() + 30)/3 + 30;
    local hit = str * pCharData:GetAtk_STR() + 30;
	local def = con * pCharData:GetDef_CON();
	local eva = agi * pCharData:GetEva_AGI()+10;
	local power = spr * pCharData:GetPwd_SPR() + sta * pCharData:GetPwd_STA() + con * pCharData:GetPwd_CON() + str * pCharData:GetPwd_STR();
...
function CalSkillNextLvExp(Lv, shopID)
	...
	elseif shopID == 43 then
		local yirong = 50000 + (Lv - 1) * (Lv - 1) * 100000
		LuaI.SetTempResult(yirong);
	elseif shopID == 44 then
		local shiwei = 250000 * Lv
		LuaI.SetTempResult(shiwei);
	end
	
end

function CalSkillNextLvMoney(Lv, shopID)
	local result = 0.3 * Lv^3 + 10 ;	
	if shopID >= 1 and shopID <= 12 then       
	    LuaI.SetTempResult(result);
	elseif shopID >= 13 and shopID <= 17 then       
	    LuaI.SetTempResult(2*result);
	elseif shopID >= 18 and shopID <= 29 then
	    LuaI.SetTempResult(result);
	elseif shopID >= 30 and shopID <= 41 then
	    LuaI.SetTempResult(result);
...
function CalQuestionRewards(pCharacter,point,rank)
	...
	if rank >= 1 and rank <= 10 then
		...
		pCharacter:GetScript():Rumor(string.format("\#W%s\#O在“大李小百科”活动中展示出过人的智慧，奖励物品\#P%s\#O，以此鼓励。", pCharacter:GetName(), GetItemName(ItemId)), false);
...
function OnCharacterEnter(pChar)
	pChar:AddBuff(40003, 0, 0, 0, 90000000, 100)
  ...
  if lv >= 10 then
    pChar:SetReborn(34, 194, 27)
  else
    pChar:SetReborn(1, 77, 53)
  end
	----由于有些情况下换线时 读取下列变身无效果，所以在OnCharacterLeave(pChar)增加另一个，修改时注意修改2处
	for bf = 20010 , 20237 do
	...
	if OnMap == 20320 and pChar:GetPK_Value() == 0 then
		pChar:JumpMap(20320,29,124,0,0,0,false);
	end	
	...
	if OnMap == 58100 then
		pChar:JumpMap(20300,71,359);
	end
	if OnMap == 20321 then
		pChar:JumpMap(20200,58,132);
	end
	--镇帮神器
...
--[[	if pChar:GetQuests():GetFlag(40001) == 0 then
	...
	--]]
	--新年活动邮件提示
	if Year == 2017 and Mon == 1 and Day == 1 then
		...
		pChar:SetSystemLetterSender("情缘西游")--不能为空
		pChar:SetSystemLetterTitle("元旦邮件提示")--不能为空
		pChar:SetSystemLetterMessage("    新年新气象，情缘西游也为玩家准备了元旦活动。1月1日-1月3日全天开放世界双倍时间,门派闯关1月1日-1月3日下午15:00-17:00任何闯,还在等待什么,赶紧叫上小伙伴们来体验吧~！")--不能为空
		pChar:SendSystemLetterData()--不能去掉
...
--[[	if Year == 2011 and Mon == 2 and Day == 7 then
	...
	--[[if pChar:GetQuests():GetFlag(1007) == 0 and pChar:GetLevel() >= 30  then
		...
		pChar:SetSystemLetterSender("紫界")--不能为空
		pChar:SetSystemLetterTitle("畅游梦古")--不能为空
		pChar:SetSystemLetterMessage("   在新资料片《十大恶人》中，不少江湖侠客包括十大恶人、十二星象、江别鹤等均已习得四相绝学，大侠们若想畅游梦古，则四相附魔符不可不备。\\r    在此，先送上这些“四相附魔符”以供各位玩家体验。“四相附魔符”可以通过山庄里的四相任务以及新活动“十恶闹江湖”中获得。")--不能为空
		pChar:SendSystemLetterData()--不能去掉
...
function OnCharacterLeave(pChar)
...
function OnCharacterJailDelivery(pChar,pTarget)
	if pChar and pTarget then
		if pTarget:GetMap_id() == 20320 and pTarget:GetQuests():GetFlag(406) == 1 then 
			...
			pTarget:JumpMap(20303,14,60,0,0,0,false);
...
function CheckJumpMap(pChar)
	local mapId = pChar:GetMap_id();
	if mapId == 20320 then
		pChar:GetScript():ShowError("天牢地图，禁止跳转");
		LuaI.SetTempResult(0);
		return;
	end
...
function UpateGuildMaintain(pGuild, success)
	...
	if pGuild:GetFund() < cost * 48 then
		...
		pGuild:SetStudy(0)
		pGuild:SendGuildMessage(string.format("山庄资金不足，扣除基本维护金%d。山庄欠缴维护费用，惹怒了包租婆，估计本山庄一、三、五要停水停气，二、四、六要间断电断网了……",MaintainCost))
		return
	end
	if success == true then
		...
		if pGuild:GetArtifactType() == 3 then
			pGuild:SendGuildMessage(string.format("山庄成功维护，受镇帮神器太阿影响，只扣除维护资金%d", MaintainCost))
		else
			pGuild:SendGuildMessage(string.format("山庄成功维护，扣除维护资金%d", pGuild:GetMaintainCost()))
		end
		...
		if pGuild:GetArtifactType() == 1 then
			...
			pGuild:SendGuildMessage("帮派资材、矿产、杂材、皮革得到补充")
		end
...
function CalSingleWarReward(pChar, result, enemyCount, turnCount, pTarget)
	...
	if result == 0 then --win
		...
		Exp =  (4*PartyLevel+10)*6.6*70*0.50 * 10;--论剑获胜经验10倍
		SingleWarPoint = 30;
	...
	elseif result == 5 then --battle lose
		...
		Exp = (4*PartyLevel+10)*6.6*70*0.2 * 10;--论剑获胜经验10倍
		pChar:GetScript():ShowError(string.format("\#W积极参与论剑，获得经验值%d",Exp));
		if math.random(1,1) == 1 and pChar:GetItems():GetFreeSlot(0) > 0 then
			--pChar:GetScript():Rumor(string.format("真是鸿运当头，\#W%s\#O积极参与论剑，获得\#W马车票\#O的额外奖励。",pChar:GetName()));
			--pChar:GetItems():AddItem(51073,1)
		end
	else 	--leave the battle
	end
	pChar:ChangeExpForScript(Exp);
...
function DonateMoney(pChar, money, maxFund)
	if pChar:HasGuild() then
		...
		if TotalRound560 > 100 then
			pChar:GetScript():ShowError("感谢您为帮派发展积极捐献，但一周内只能捐献一百次，要想继续捐献请下周再来。");
			LuaI.SetTempResult(0);
			return
		end
		...
		if pGuild ~= nil then
			...
			if pGuild:GetFund() >= maxFund * 0.8 then
				...
				LuaI.SetTempResult(0);
			elseif reallyMoney < minMoney then--低于山庄规模 * 1W
				pChar:GetScript():ShowError(string.format("对不起，%d级山庄捐款不得低于%d，捐款不要这么小气嘛，请慷慨解囊。", pGuild:GetManorScale(), minMoney));
				LuaI.SetTempResult(0);
...
function CheckManorBuildScale(pChar)
	...
	if pChar ~= nil and pChar:HasGuild() then
		local pGuild = pChar:GetGuild()
		if pGuild ~= nil then
			local ScaleLv		= pGuild:GetManorScale()	--山庄等级
			...
			local ElementLv		= pGuild:GetElement()		--四相房等级
			local Count			= 0
			if ScaleLv >= 5 then
			pChar:GetScript():ShowError("当前山庄规模已达上限，无法继续提升");
				return
			end
			...
			if Count < 2 then
				pChar:GetScript():ShowHint(string.format("\#W至少\#G两\#W种建筑数量达到当前上限（书院、福利房、金库、四相房当前上限\#G%d\#W级，厢房当前上限\#G%d\#W级）才能提升规模", ScaleLv * 4, ScaleLv * 3));
				pChar:GetScript():ShowError(string.format("\#W至少两种建筑数量达到当前上限（书院、福利房、金库、四相房当前上限%d级，厢房当前上限%d级）才能提升规模", ScaleLv * 4, ScaleLv * 3));
			else
				LuaI.SetTempResult(1);
			end		
		end
