...
function skill5048CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill5048Use(Atk, Target, battle, skillLv)
    local targetlist = {};
	...
	while targetlist[i] ~= nil do
		...
		local AtkEffect = (1 - 0.05) * targetlist[i]:GetATK();
		local PowEffect = (1 - 0.05) * targetlist[i]:GetPOW();
		local DefEffect = (1 - 0.05) * targetlist[i]:GetDEF();
		
		if math.random(1, 100) <= 33 and not targetlist[i]:IsBuffExist(1408) then
			targetlist[i]:AddBuff(1408, (Lv + 10), AtkEffect, 0, turn, 100);
		elseif math.random(1, 100) <= 66 and not targetlist[i]:IsBuffExist(1409) then
			targetlist[i]:AddBuff(1409, (Lv + 10), PowEffect, 0, turn, 100);
		elseif math.random(1, 100) <= 100 and not targetlist[i]:IsBuffExist(1410) then
			targetlist[i]:AddBuff(1410, (Lv + 10), DefEffect, 0, turn, 100);
		end
