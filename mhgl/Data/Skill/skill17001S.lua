...
function skill17001CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	...
	if Atk:GetHP() < reqHp then
		local Msg = "\#W气血不足\#G20\%\#W，无法施展移形换影";
		Atk:SkillFail(Msg);
		do return end
	elseif Atk:GetHP() < consumeHp then
		local Msg1 = string.format("\#W气血不足\#G%d\#W，无法施展移形换影", consumeHp);
		Atk:SkillFail(Msg1);
		do return end
	elseif Atk:GetSP() < consumeSp then
		local Msg2 = string.format("\#W魔法不足\#G%d\#W，无法施展移形换影", consumeSp);
		Atk:SkillFail(Msg2);
		do return end
	elseif Atk:HasParty() then
		local Msg = "\#W组队状态下无法施展移形换影";
		Atk:SkillFail(Msg);
		do return end
...
function skill17001Use(Atk, Target, battle, skillLv)
	...
	if math.random(1, 100) <= Rate then
		Atk:JumpMap(10, 11, 124);
	else
		Atk:SkillFail("\#W学艺不精，移形换影施展失败");
	end
