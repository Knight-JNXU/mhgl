...
function skill18016CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill18016Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 0.4 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展失忆符", consumeSp);
	...
	if Target:IsBuffExist(1121) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了失忆符");
		do return end
	end
