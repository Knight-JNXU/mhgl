...
function skill21032CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此技能需潇湘仙雨修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill21032Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展缥缈式", consumeSp);
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			local randomDam = 1.5 - math.random(100, 300) / 1000;
			...
			local FinalDam = ((0.5 * Atk:GetATK() - FinalDef + weaponDam + 2500) * wDam - 250) * randomDam;
			local CritHit = 0;
			local CritRate =  math.floor(Atk:GetSkillLv(21030) / 10) + 2;
			if Atk:GetSkillLv(394) > 0 then --法宝
			   if i < 3 then
			   FinalDam = FinalDam + (FinalDam * Atk:GetSkillLv(394) * 0.2)
			   end
			end
