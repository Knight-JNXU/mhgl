...
function skill4262CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4262Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：酒意不足\#G%d\#W，无法施展流星追魂", consumeDp);
	...
	if math.random(1,100) <= 5 then
		FinalDam = ((Atk:GetATK() - FinalDef + weaponDam + 250 + Atk:GetLevel() * Atk:GetWC() / 80) * wDam - 250) * randomDam * critDam;
		CritHit = 1;
	else
		FinalDam = ((Atk:GetATK() - FinalDef + weaponDam + 250 + Atk:GetLevel() * Atk:GetWC() / 80) * wDam - 250) * randomDam;
	end
