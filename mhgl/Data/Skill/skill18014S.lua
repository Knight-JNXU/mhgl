...
function skill18014CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill18014Use(Atk, Target, battle, skillLv)

	local consumeSp = 0.4 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展失魂符", consumeSp);
	...
	if Target:IsBuffExist(1105) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了失魂符");
		do return end
	end
	...
	if math.random(1, 100) > HitRate then
		...
		if Target:IsBuffExist(3093) then
			Atk:AddBuff(1105, skillLv, 0, 0, turn, 100);
		end
		do return end
	end
	...
	if result == 2 then
		...
		Target:AddBuff(1105, skillLv, DefEffect, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
