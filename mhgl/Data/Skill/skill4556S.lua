...
function skill4556CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill4556Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展八凶法阵", consumeSp);
	...
	if lianji == 1 then
		...
		while targetlist[j] ~= nil do
			...
			local FinalDam = (lv*3+Atk:GetATK()*0.15+159+(Atk:GetPOW() - targetlist[j]:GetPOW()))*(nDam1-nDam3)* (1.7-0.1*count)*RateDam+nDam2-nDam4 ;
		...
		while targetlist[j] ~= nil do
		    ...
		    if Atk:HasSkill(9779) then
				...
				if HitRate < 10 and targetlist[j]:IsBuffExist(1124) ~= true and targetlist[j]:GetHP() ~= 0 then 
					targetlist[j]:AddBuff(1124, lv, effectATK, 0, 1, 100);
				end
