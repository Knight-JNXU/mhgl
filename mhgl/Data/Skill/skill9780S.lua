...
function skill9780CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill9780Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展地狱烈火", consumeSp);
	...
	if lianji == 1 then
		...
		while targetlist[j] ~= nil do
			...
			local FinalDam = (lv*2+Atk:GetATK()*0.1+29+(Atk:GetPOW() - targetlist[j]:GetPOW()))*(nDam1-nDam3)* (1-0.1*count)*RateDam+nDam2-nDam4 ;
		  ...
		  if Atk:HasSkill(9779) then
			...
			targetlist[j]:ChangeHp(-1 * (FinalDam + sh)*1.3);
		...
		while targetlist[j] ~= nil do
			...
			while i < lianji do
				...
				local FinalDam = ((lv*2+Atk:GetATK()*0.1+29+(Atk:GetPOW() - targetlist[j]:GetPOW()))*(nDam1-nDam3)* (1-0.1*count)*RateDam+nDam2-nDam4)*0.75 ;
		    ...
		    if Atk:HasSkill(9779) then
			   sh = Atk:GetSPR()
				...
				if CritHit1[j] == 1 then
					targetlist[j]:AddAttriChangeToLst(-1 * (FinalDam+sh)*1.3, 0, 0, 0, LuaI.BATCMD_CRIT_HIT);
				else
					targetlist[j]:AddAttriChangeToLst(-1 * (FinalDam+sh)*1.3, 0, 0, 0, LuaI.BATCMD_SKILL_HIT);
				end
