...
function skill19045CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill19045Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展救死扶伤", consumeSp);
	...
	while targetlist[i] ~= nil do
		local rec1 = (3 - 0.1 * count) * skillLv + 0.1 * skillDam + 
					0.02 * targetlist[i]:GetMaxHP() + 0.15 * (Atk:GetPOW() + Atk:GetWeaponATK());
		local rec2 = (1.3 - 0.05 * count) * skillLv + 0.1 * skillDam + 0.1 * Atk:GetPOW();
		rec1 = rec1 / 3;
		if Atk:GetSkillLv(394) > 0 then --法宝
		   rec1 = rec1 + (rec1 * Atk:GetSkillLv(394) * 0.05)
		   rec2 = rec2 + (rec2 * Atk:GetSkillLv(394) * 0.05)
		end
		if rec1 <= 1 then
			rec1 = 1;
		end
		...
		targetlist[i]:ChangeMHp(rec2*3);
		targetlist[i]:ChangeHp(rec1*3);
