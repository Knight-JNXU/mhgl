...
function skill10011CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需呼风唤雨修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill10011Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展龙卷雨击",consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		local hpChange = ((Atk:GetPOW() - (1 - fabao) * targetlist[i]:GetPOW()) + weaponDam +2500+ skillLv * 2.5)*(20-xishu)/25* nDam-500;
