...
function skill8071CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill8071Use(Atk, Target, battle, skillLv)
	...
	local MSSLv = math.floor((Lv - 35)/ 20) * 20 + 35;
	local consumeSp = 10 * ((0.5 * MSSLv + 10) / maxTargetCount) * count;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展天外飞仙", consumeSp);
