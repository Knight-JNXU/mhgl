...
function skill4409CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4409Use(Atk, Target, battle, skillLv)

	local sLv = Atk:GetLevel();
	local targetlist = {};
	local maxTargetCount = math.random(5, 10);
	...
	local i = 0;
	local buffRate = 88;
	if Target:IsMob() and not Target:IsParnter() and math.random(1, 100) < 80 then
		i = 1;
		buffRate = 68;
	end
	while targetlist[i] ~= nil do
		...
		if sLv > targetlist[i]:GetLevel() then
			HitRate = buffRate + 2.5 * (sLv - targetlist[i]:GetLevel()) ^ 0.8;
		else
			HitRate = buffRate + 3.2 * (sLv - targetlist[i]:GetLevel());
		end
		
		if math.random(1, 100) <= HitRate then
			targetlist[i]:AddBuff(1706, sLv, Apos, 0, 8, 100);
		else
