...
function skill17021CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill17021Use(Atk, Target, battle, skillLv)

	local consumeSp = 35;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展如花解语", consumeSp);
	
	if Atk:GetSP() < consumeSp then
		Atk:SkillFail(Msg);
		do return end
	end
	if Target:IsBuffExist(1101) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了如花解语")
		do return end
	end
	...
	if math.random(1, 100) > HitRate then
		...
		if Target:IsBuffExist(3093) then
			Atk:AddBuff(1101, skillLv, 0, 0, turn, 100);
		end
		do return end
	end	
	...
	if result == 2 then
		...
		Target:AddBuff(1101, skillLv, 0, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
end
