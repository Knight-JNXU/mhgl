...
function skill19064CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill19064Use(Atk, Target, battle, skillLv)
	
	if Target:IsBuffExist(1504) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经处在金刚护体状态");
		do return end
	end
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展金刚护体", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		targetlist[i]:AddBuff(1504, skillLv, effectAtk, 0, turn, 100);
