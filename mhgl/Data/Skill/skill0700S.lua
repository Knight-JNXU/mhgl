...
function skill0700CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill0700Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展弹指神通", consumeSp);
	...
	if lianji == 1 then
		...
		while targetlist[j] ~= nil do
			...
			local FinalDam = 1.2 * ((Atk:GetPOW() - targetlist[j]:GetPOW() + (3.4 - 0.3 * count) * lv + nDamsBonus1 * lv + 250) * nDam - 250) * RateDam;
		...
		while targetlist[j] ~= nil do
			...
			while i < lianji do
				...
				local FinalDam = 1.2 * ((Atk:GetPOW() - targetlist[j]:GetPOW() + (3.4 - 0.3 * count) * lv + nDamsBonus1 * lv + 250) * nDam - 250) * 0.75 * RateDam;
