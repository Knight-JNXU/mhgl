...
function skill0218CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill0218Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展翻江倒海", consumeSp);
	...
	if lianji == 1 then
		...
		while targetlist[j] ~= nil do
			...
			local FinalDam = (lv*3+Atk:GetATK()*0.11+109+(Atk:GetPOW() - targetlist[j]:GetPOW()))*(nDam1-nDam3)* (1.5-0.1*count)*RateDam+nDam2-nDam4 ;
		...
		while targetlist[j] ~= nil do
		    ...
		    if Atk:HasSkill(9779) then
				...
				if CritHit1[j] == 1 then
					targetlist[j]:AddAttriChangeToLst(-1 * (FinalDam+sh+120)*1.3, 0, 0, 0, LuaI.BATCMD_CRIT_HIT);
				else
					targetlist[j]:AddAttriChangeToLst(-1 * (FinalDam+sh+120)*1.3, 0, 0, 0, LuaI.BATCMD_SKILL_HIT);
				end
				...
				if HitRate < 10 and targetlist[j]:IsBuffExist(1407) ~= true and targetlist[j]:GetHP() ~= 0 then 
					targetlist[j]:AddBuff(1407, lv, effectATK, 0, 3, 100);
				end
