...
function skill4260CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4260Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：愤怒不足\#G%d\#W，无法施展破血狂攻", consumeDp);
	...
	while i <= 2 do
		...
		if math.random(1,100) <= 5 then
			FinalDam[i] = ((DamRate[i] * Atk:GetATK() - FinalDef + weaponDam + 250 + Atk:GetLevel() * Atk:GetWC() / 65) * wDam - 250) * randomDam * critDam;
			CritHit[i] = 1;
		else
			FinalDam[i] = ((DamRate[i] * Atk:GetATK() - FinalDef + weaponDam + 250 + Atk:GetLevel() * Atk:GetWC() / 65) * wDam - 250) * randomDam;
		end
