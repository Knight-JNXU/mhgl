...
function skill21052CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill21052Use(Atk, Target, battle, skillLv)

	if Target:IsBuffExist(3067) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经处在天地同寿状态");
		do return end
	end
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展天地同寿", consumeSp);
	...
	local turn = 1 + math.floor((skillLv - 10) / 10);
	...
	while targetlist[i] ~= nil do
		...
		targetlist[i]:AddBuff(3067, skillLv, effectAtk, 0, turn, 100);
