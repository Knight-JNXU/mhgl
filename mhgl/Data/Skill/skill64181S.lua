...
function skill64181CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需降魔心经修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill64181Use(Atk, Target, battle, skillLv)
...
local targetlist = {};
	...
	local consumeSp = (1.8 * skillLv) * count;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展如来神掌",consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		local hpChange = (0.7 * (Atk:GetPOW() - 0.5 * targetlist[i]:GetPOW()) + 0.35 * weaponDam + (1.6 - 0.2 * count) 
		...
		Atk:AddBuff(1527, skillLv, 0, 0, 1, 100);
