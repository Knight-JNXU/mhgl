...
function skill24021CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需阴风绝章修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill24021Use(Atk, Target, battle, skillLv)

	local consumeSp = 40;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展夺魄令", consumeSp);
	
	if Atk:GetSP() < consumeSp then
		Atk:SkillFail(Msg);
		do return end
	end
	if Target:IsBuffExist(1128) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了夺魄令")
		do return end
	end
	...
	if skillLv > Target:GetLevel() then
		HitRate = (65 + 2 * (skillLv - Target:GetLevel()) ^ 0.5) * nDam;
	else
		HitRate = (65 + 2.2 * (skillLv - Target:GetLevel())) * nDam;
	end
	...
	if math.random(1, 100) > HitRate then
		...
		if Target:IsBuffExist(3093) then
			Atk:AddBuff(1128, skillLv, 0, 0, turn, 100);
		end
		do return end
	end
	...
	if result == 2 then
		...
		Target:AddBuff(1128, skillLv, 0, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
end
