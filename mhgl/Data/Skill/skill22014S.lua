...
function skill22014CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill22014Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展浪涌", consumeSp);
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if math.random(1,100) <= 5 then
				FinalDam =((1.1*Atk:GetATK() - targetlist[i]:GetDEF() + 250) * wDam - 250)  * randomDam * critDam;
				CritHit = 1;
			else
			    FinalDam =((1.1*Atk:GetATK() - targetlist[i]:GetDEF() + 250) * wDam - 250) * randomDam ;
				end
