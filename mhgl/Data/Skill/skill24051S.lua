...
function skill24051CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需地冥妙法修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill24051Use(Atk, Target, battle, skillLv)
	
	local targetlist = {};
	local maxTargetCount = math.floor((skillLv - 80) / 25);
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展地涌金莲", consumeSp);
	...
	while targetlist[i] ~= nil do
		local rec1 = (3 - 0.1 * count) * skillLv + 0.1 * skillDam + ((Atk:GetSkillLv(378)+10)*12) + 0.02 * targetlist[i]:GetMaxHP() + 0.15 * (Atk:GetPOW() + Atk:GetWeaponATK());
		local rec2 = (1.3 - 0.05 * count) * skillLv + 0.1 * skillDam + ((Atk:GetSkillLv(378)+10)*12) + 0.1 * Atk:GetPOW();
		rec1 = rec1*2.2
		rec2 = rec2*2.2
		if Atk:GetSkillLv(394) > 0 then --法宝
		   rec1 = rec1 + (rec1 * Atk:GetSkillLv(394) * 0.02)
		   rec2 = rec2 + (rec2 * Atk:GetSkillLv(394) * 0.02)
		end
		if Atk:IsBuffExist(8102) then
	      rec1 = rec1 + (rec1 * 0.2)
	    end
		local CritRate = math.random(1,100)
		local CritRate2 = math.random(170,190)/100
		if Atk:IsBuffExist(8202) then
		   if CritRate <= 35 then
		      rec1 = rec1*CritRate2
		   end
	    end
