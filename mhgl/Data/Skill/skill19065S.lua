...
function skill19065CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill19065Use(Atk, Target, battle, skillLv)
	
	if Target:IsBuffExist(1503) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经处在一苇渡江状态");
		do return end
	end
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展一苇渡江", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		targetlist[i]:AddBuff(1503, skillLv, effectAtk, 0, turn, 100);
