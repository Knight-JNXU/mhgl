...
function skill19062CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill19062Use(Atk, Target, battle, skillLv)
	
	if Target:IsBuffExist(1505) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经处在金刚护法状态");
		do return end
	end
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展金刚护法", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		targetlist[i]:AddBuff(1505, skillLv, effectAtk, 0, turn, 100);
