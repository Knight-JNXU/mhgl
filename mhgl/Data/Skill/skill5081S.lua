...
function skill5081CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill5081Use(Atk, Target, battle, skillLv)
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if math.random(1,100) <= CritRate then
				FinalDam = ((1.05 * Atk:GetATK() - FinalDef + 250) * wDam - 350) * randomDam * critDam;
				CritHit = 1;
			else
				FinalDam = ((1.05 * Atk:GetATK() - FinalDef + 250) * wDam - 350) * randomDam;
			end
			...
			if result == 2 then
				...
				targetlist[i]:AddAttriChangeToLst(-0.9 * FinalDam, 0, 0, 0, LuaI.BATCMD_SKILL_HIT);
			end
