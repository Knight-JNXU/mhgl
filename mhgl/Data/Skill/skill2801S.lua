...
function skill2801CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	...
	if Atk:GetHP() < reqHp then
		local Msg = "\#W生命不足\#G20\%\#W，无法施展飞燕步";
		Atk:SkillFail(Msg);
		do return end
	elseif Atk:GetHP() < consumeHp then
		local Msg1 = string.format("\#W生命不足\#G%d\#W，无法施展飞燕步", consumeHp);
		Atk:SkillFail(Msg1);
		do return end
	elseif Atk:GetSP() < consumeSp then
		local Msg2 = string.format("\#W内力不足\#G%d\#W，无法施展飞燕步", consumeSp);
		Atk:SkillFail(Msg2);
		do return end
	elseif Atk:HasParty() then
		local Msg = "\#W组队状态下无法施展飞燕步";
		Atk:SkillFail(Msg);
		do return end
...
function skill2801Use(Atk, Target, battle, skillLv)
	...
	if math.random(1, 100) <= Rate then
		Atk:JumpMap(30200, 26, 156);
	else
		Atk:SkillFail("\#W学艺不精，飞燕步施展失败");
	end
