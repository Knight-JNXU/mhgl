...
function skill0352CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill0352Use(Atk, Target, battle, skillLv)
	...
	local consumeMoney = 100 * count;
	
	local Msg = string.format("\#W[战斗讯息]：银两不足\#G%d\#W，无法施展一掷千金",consumeMoney)
	local Msg1 = string.format("\#W[战斗讯息]：施展一掷千金成功，扣除银两\#G%d\#W两",consumeMoney)
	...
	if lianji == 1 then
		...
		while targetlist[j] ~= nil do
			...
			local FinalDam = (Atk:GetPOW() - targetlist[j]:GetPOW() + (3.0 - 0.2 * count) * lv) * RateDam;
			...
			if HitRate < 20 and targetlist[j]:IsBuffExist(1405) ~= true and targetlist[j]:GetHP() ~= 0 then
				targetlist[j]:AddBuff(1405, lv, effectSPD, 0, 3, 100);
			end
		...
		while targetlist[j] ~= nil do
			...
			while i < lianji do
				...
				if HitRate < 8 and targetlist[j]:IsBuffExist(1405) ~= true and targetlist[j]:GetHP() ~= 0 then
					targetlist[j]:AddBuff(1405, lv, effectSPD, 0, 3, 100);
				end
