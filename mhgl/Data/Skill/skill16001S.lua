...
function skill16001CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	...
	if Atk:GetHP() < reqHp then
		local Msg = "\#W生命不足\#G20\%\#W，无法施展腾云驾雾";
		Atk:SkillFail(Msg);
		do return end
	elseif Atk:GetHP() < consumeHp then
		local Msg1 = string.format("\#W生命不足\#G%d\#W，无法施展腾云驾雾", consumeHp);
		Atk:SkillFail(Msg1);
		do return end
	elseif Atk:GetSP() < consumeSp then
		local Msg2 = string.format("\#W魔法不足\#G%d\#W，无法施展腾云驾雾", consumeSp);
		Atk:SkillFail(Msg2);
		do return end
	elseif Atk:HasParty() then
		local Msg = "\#W组队状态下无法施展腾云驾雾";
		Atk:SkillFail(Msg);
		do return end
	...
	elseif Atk:GetMap_id() == 252 then
		Atk:SkillFail("\#W当前地图不能使用回门派技能");
	end
end

function skill16001Use(Atk, Target, battle, skillLv)
	...
	if math.random(1, 100) <= Rate then
		Atk:JumpMap(13, 132, 351);
	else
		Atk:SkillFail("\#W学艺不精，腾云驾雾施展失败");
	end
