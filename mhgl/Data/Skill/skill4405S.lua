...
function skill4405CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4405Use(Atk, Target, battle, skillLv)
	
	local sLv = Target:GetLevel();
	local consumeSp = 1.0 * sLv;
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展神·大悲绝魂", consumeSp);
	...
	while i <= lianji do
		...
		if math.random(1,100) <= CritRate then
			FinalDam[i] = ((1.52 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * critDam * DamRate[i];
			CritHit[i] = 1;
		else
			FinalDam[i] = ((1.52 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * DamRate[i];
		end
		...
		if Target:GetCommand() == LuaI.BATCMD_DEFEND then
			...
			if FinalDam[i] <= 15 then
				FinalDam[i] = math.random(1, 15);
			end
		...
		if math.random(1,100) <= buffRate and not Target:IsBuffExist(1705) then
			Target:AddBuff(1705, sLv, 0, 0, 15, 100);
		end
		i = i + 1;
	end
