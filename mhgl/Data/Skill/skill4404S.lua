...
function skill4404CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4404Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展神·雷霆震怒", consumeSp);
	...
	if 1.21 * Atk:GetATK() > Target:GetDEF() then
	...
	while i <= lianji do
		...
		if math.random(1,100) <= CritRate then
			FinalDam[i] = ((1.25 * Atk:GetATK() - FinalDef + 350) * wDam - 150) * randomDam * critDam * DamRate[i];
			CritHit[i] = 1;
		--	buffRate = 40;
		else
			FinalDam[i] = ((1.25 * Atk:GetATK() - FinalDef + 350) * wDam - 150) * randomDam * DamRate[i];
		end
		...
		if math.random(1,100) <= buffRate and not Target:IsBuffExist(1123) then
			Target:AddBuff(1123, sLv, 0, 0, 3, 100);
		end
		i = i + 1;
	end
