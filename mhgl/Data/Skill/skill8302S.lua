...
function skill8302CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill8302Use(Atk, Target, battle, skillLv)
	...
	--for i = 0, targetcounter  - 1 do
		...
		local hpChange = (Atk:GetPOW() - targetlist[i]:GetPOW() + 
						(2.5 - 0.3 * math.floor((Lv - 25) / 25)) * Lv + 7.0 * Lv )  * 1.05 ^ nDam;
		if hpChange <= 10 then
