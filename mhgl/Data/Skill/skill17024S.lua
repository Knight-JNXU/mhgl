...
function skill17024CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill17024Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 40;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展娉婷嬝娜", consumeSp);
	...
	if Target:IsBuffExist(7025) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了娉婷嬝娜");
		do return end
	end
	...
	if math.random(1, 100) > HitRate then
		...
		if Target:IsBuffExist(3093) then
			Atk:AddBuff(7025, skillLv, 0, 0, turn, 100);
		end
		do return end
	end
	...
	if result == 2 then
		...
		Target:AddBuff(7025, skillLv, 0, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
