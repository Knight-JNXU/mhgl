...
function skill1412CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill1412Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展伤寒掌", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		if not targetlist[i]:IsBuffExist(1408) then 
			...
			local AtkEffect = (1 - 0.075) * targetlist[i]:GetATK();
			local Rate = 75;
			if math.random(1, 100) < Rate then
				targetlist[i]:AddBuff(1408, skillLv, AtkEffect, 0, turn, 100);
			end
		end
