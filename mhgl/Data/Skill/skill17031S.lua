...
function skill17031CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需玉质冰肌修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill17031Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 30;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展百毒不侵", consumeSp);
	...
	local buffid  = {1000,1001,1005,1540,1541};	
	local rate    = {0,0};
	local turnrate = {0,0};
	local hitRate = 0;
	local rateMax = 0;
