...
function skill0037CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill0037Use(Atk, Target, battle, skillLv)
	local targetlist = {};
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展立劈华山", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		if targetlist[i]:IsPlayer() or targetlist[i]:IsParnter() then
			...
			local HitEffect = (1 - 0.15 * skillLv / 100) * Target:GetHIT();
		
			Target:AddBuff(61, skillLv, HitEffect, 0, turn, 100);
		end	
