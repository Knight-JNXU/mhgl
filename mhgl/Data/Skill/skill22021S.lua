...
function skill22021CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需诛魔修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill22021Use(Atk, Target, battle, skillLv)
	--Sp=0.3 * 技能等级 +10
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展腾雷", consumeSp);
	...
	if 1.08 * Atk:GetATK() > Target:GetDEF() then
	...
	if math.random(1,100) <= 5 then
		FinalDam = ((1.21 * Atk:GetATK() - FinalDef + weaponDam + 350) * wDam - 200) * randomDam * critDam;
		CritHit = 1;
	else
		FinalDam = ((1.21 * Atk:GetATK() - FinalDef + weaponDam + 350) * wDam - 200) * randomDam;	
	end
	...
	if Target:GetHP() ~= 0 and (Target:IsBuffExist(1112) == false) then
		Target:AddBuff(1112, skillLv, 0, 0, 5, 100);
	end
