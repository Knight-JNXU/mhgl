...
function skill1832CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	...
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需阴阳二气诀修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill1832Use(Atk, Target, battle, skillLv)
	--Sp=0.3* 技能等级 +10
	local consumeHp = 0.3 * skillLv +10;
	if Atk:IsBuffExist(1702) then
		consumeHp = consumeHp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：气血不足\#G%d\#W，无法施展魔息术", consumeHp);
	...
	local spChange = 0.75 * skillLv + 15;
