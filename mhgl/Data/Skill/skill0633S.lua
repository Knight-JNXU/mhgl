...
function skill0633CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill0633Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展夺命十三剑", consumeSp);
	...
	while targetlist[j] ~= nil do
		...
		if math.random(1,100) <= 10 and targetlist[j]:IsBuffExist(1008) ~= true and targetlist[j]:GetHP() ~= 0 then 
			targetlist[j]:AddBuff(1008, lv, 0, 0, 3, 100);
		end
