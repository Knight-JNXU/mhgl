...
function skill16031CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需宁气诀修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill16031Use(Atk, Target, battle, skillLv)
	--Sp=0.2 * 技能等级 +10
	local consumeSp = 0.4 * skillLv +10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展宁心", consumeSp);
	...
	local buffid = {1101, 1104, 1109, 1120};	--对应skillid
	local rate = {0, 0, 0, 0};
	...
	if turnrateMax > 0 then
		...
		if  hitRate == 100 then
			...
			Target:AddBuff(1604, skillLv, 0, 0, turn, 100);
			allfail = 0;
		end
