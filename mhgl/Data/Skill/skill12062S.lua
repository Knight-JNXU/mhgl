...
function skill12062CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill12062Use(Atk, Target, battle, skillLv)

	local consumeDp = 150;
	if Atk:IsBuffExist(1702) then
		consumeDp = consumeDp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：酒意不足%d，无法施展冤魂不散", consumeDp);
