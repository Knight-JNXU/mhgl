...
function skill2051CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill2051Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 0.3 * skillLv + 10;
	local consumeHp = 0.15 * Atk:GetHP();
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展血祭幽魂", consumeSp);
	...
	if math.random(1,100) <= CritRate then
		FinalDam = 1.0 * ((Atk:GetATK() - FinalDef + weaponDam + 250) * wDam - 250) * randomDam * critDam;
		CritHit = 1;
	else
		FinalDam = 1.0 * ((Atk:GetATK() - FinalDef + weaponDam + 250) * wDam - 250) * randomDam;	
	end
	...
	Atk:AddBuff(1519, skillLv, 0, 0, 4, 100);
