...
function skill4544CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4544Use(Atk, Target, battle, skillLv)
	
	local sLv = Atk:GetLevel();
	local consumeHp = 0.2 * Atk:GetHP();
	local Msg = string.format("\#W[战斗讯息]：生命不足\#G%d\#W，无法施展血碎长空", consumeHp);
	local reqHp = Atk:GetMaxHP() * 0.3;
	if Atk:GetHP() < reqHp then
		Atk:SkillFail("\#W[战斗讯息]：生命不足\#G30\%\#W，无法施展血碎长空");
		do return end
	end
	...
	while i <= lianji do
		...
		if math.random(1,100) <= CritRate then
			FinalDam[i] = ((1.1 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * critDam * DamRate[i];
			CritHit[i] = 1;
		else
			FinalDam[i] = ((1.1 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * DamRate[i];
		end
