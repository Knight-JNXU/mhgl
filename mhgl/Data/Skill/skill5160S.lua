...
function skill5160CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill5160Use(Atk, Target, battle, skillLv)
	if Target:IsBuffExist(1102) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了闭穴绝手");
		do return end
	end
	...
	if result == 2 then
		...
		Target:AddBuff(1102, (Lv + 10), 0, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
