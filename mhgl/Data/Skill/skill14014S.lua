...
function skill14014CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	...
	elseif Atk:IsBuffExist(1502) == false then
		Atk:SkillFail("\#W[战斗讯息]：此招式需先变身后才能施展");
	end	
end

function skill14014Use(Atk, Target, battle, skillLv)
	--Sp=0.3 * 技能等级 +10
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展象形", consumeSp);
	...
	if Target:GetHP() ~= 0 and (Target:IsBuffExist(1108) == false) then
		Target:AddBuff(1108, skillLv, 0, 0, 1, 100);
	end
	
	Atk:RemoveBuff(1502);
	Atk:AddBuff(1809, skillLv, 0, 0, 1, 100);
