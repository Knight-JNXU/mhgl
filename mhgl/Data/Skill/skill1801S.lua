...
function skill1801CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	...
	if Atk:GetHP() < reqHp then
		local Msg = "\#W气血不足\#G20\%\#W，无法施展振翅千里";
		Atk:SkillFail(Msg);
		do return end
	...
	elseif Atk:GetSP() < consumeSp then
		local Msg2 = string.format("\#W魔法不足\#G%d\#W，无法施展振翅千里", consumeSp);
		Atk:SkillFail(Msg2);
		do return end
...
function skill1801Use(Atk, Target, battle, skillLv)
