...
function skill22042CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill22042Use(Atk, Target, battle, skillLv)
	
	if Target:IsBuffExist(2043) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经处在镇魂诀状态");
		do return end
	end
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展镇魂诀", consumeSp);
...
if Target:IsBuffExist(1491) then
		Target:RemoveBuff(1491);
	elseif Target:IsBuffExist(1491) then
		Target:RemoveBuff(1491);
		Target:AddBuff(2043, skillLv, 0, 0, 6, 100);
	else
		Target:AddBuff(2043, skillLv, 0, 0, 6, 100);
	end
