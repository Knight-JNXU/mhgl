...
function skill1414CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill1414Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展软骨掌", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		if not targetlist[i]:IsBuffExist(1410) then 
			...
			if math.random(1, 100) < Rate then
				targetlist[i]:AddBuff(1410, skillLv, DefEffect, 0, turn, 100);
			end
		end
