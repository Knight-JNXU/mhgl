...
function skill16022CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill16022Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 50;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展金刚镯", consumeSp);
	...
	if Target:IsBuffExist(5013) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了金刚镯");
		do return end
	end
	...
	if math.random(1, 100) > HitRate then
		...
		if Target:IsBuffExist(3093) then
			Atk:AddBuff(5013, skillLv, 0, 0, turn, 100);
		end
		do return end
	end
	...
	if result == 2 then
		...
		Target:AddBuff(5013, skillLv, 0, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
