...
function skill19051CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill19051Use(Atk, Target, battle, skillLv)
	
	local consumeSp = 0.4 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展正骨", consumeSp);
	...
	local rec1 = 4.6 * skillLv + 0.1 * skillDam + 0.02 * Target:GetMaxHP() + ((Atk:GetSkillLv(378)+7)*5) + 0.15 * (Atk:GetPOW() + Atk:GetWeaponATK());
