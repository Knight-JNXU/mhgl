...
function skill0172CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill0172Use(Atk, Target, battle, skillLv)
	local targetlist = {};
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展封穴金针", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		local effectPow = (1 - (0.2 - 0.03 * math.floor((skillLv - 35) / 25)) * skillLv / 100) * targetlist[i]:GetPOW();
		
		if effectPow <= 1 then
			effectPow = 1;
		end
		
		targetlist[i]:AddBuff(47, skillLv, effectPow, 0, turn, 100);
