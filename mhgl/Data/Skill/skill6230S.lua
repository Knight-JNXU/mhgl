...
function skill6230CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill6230Use(Atk, Target, battle, skillLv)
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if result == 2 then
				...
				targetlist[i]:AddAttriChangeToLst(-1 * FinalDam, -1 * FinalDam/3, 0, 0, LuaI.BATCMD_SKILL_HIT);				
			end
