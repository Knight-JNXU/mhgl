...
function skill19063CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill19063Use(Atk, Target, battle, skillLv)
	
	if Target:IsBuffExist(1501) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经处在韦陀护法状态");
		do return end
	end
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展韦陀护法", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		targetlist[i]:AddBuff(1501, skillLv, effectAtk, 0, turn, 100);
