...
function skill4401CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4401Use(Atk, Target, battle, skillLv)
	
	local sLv = Atk:GetLevel();
	local consumeSp = 1.5 * sLv;
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展神·一剑飞花", consumeSp);
	...
	if 1.18 * Atk:GetATK() > Target:GetDEF() then
	...
	while i <= lianji do
		...
		if math.random(1,100) <= CritRate then
			FinalDam[i] = ((1.15 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * critDam * DamRate[i];
			CritHit[i] = 1;
		else
			FinalDam[i] = ((1.15 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * DamRate[i];
		end
		...
		if Target:GetCommand() == LuaI.BATCMD_DEFEND then
			...
			if CritHit[i] == 1 then
				Target:AddAttriChangeToLst(-1 * FinalDam[i], -1 * FinalDam[i] / 3, 0, 0, LuaI.BATCMD_DEFEND_CRIT_HIT);
			else
				Target:AddAttriChangeToLst(-1 * FinalDam[i], -1 * FinalDam[i] / 3, 0, 0, LuaI.BATCMD_DEFEND_HIT);
			end
		else
			if FinalDam[i] <= 10 then
				FinalDam[i] = math.random(1, 10);
			...
			if CritHit[i] == 1 then
				Target:AddAttriChangeToLst(-1 * FinalDam[i], -1 * FinalDam[i] / 3, 0, 0, LuaI.BATCMD_CRIT_HIT);
			else
				Target:AddAttriChangeToLst(-1 * FinalDam[i], -1 * FinalDam[i] / 3, 0, 0, LuaI.BATCMD_NORM_HIT);
			end
		end
		i = i + 1;
	end
