...
function skill4502CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4502Use(Atk, Target, battle, skillLv)
	
	local sLv = Atk:GetLevel();
	local consumeSp = 1.5 * sLv;
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展偷天换日", consumeSp);
	...
	if math.random(1,100) <= CritRate then
		FinalDam = ((1.0 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * critDam;
		CritHit = 1;
	else
		FinalDam = ((1.0 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam;
	end
	...
	if math.random(1,100) <= 30 then
		Target:ChangeHp( FinalDam / 3);
	else
		Target:ChangeHp(-2 * FinalDam);
	end
