...
function skill8034CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill8034Use(Atk, Target, battle, skillLv)
	local Lv = Atk:GetLevel();
	local consumeSp = 10 * (0.4 * Lv +10);
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展运气解穴", consumeSp);
