...
function skill21022CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill21022Use(Atk, Target, battle, skillLv)
	
	local consumeHp = 1 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeHp = consumeHp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：生命不足\#G%d\#W，无法施展炼气化神", consumeHp);
	...
	if Target:IsBuffExist(1203) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经处在炼气化神状态");
		do return end
	end
