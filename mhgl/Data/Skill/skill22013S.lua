...
function skill22013CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill22013Use(Atk, Target, battle, skillLv)
	...
	if Atk:GetSP() < consumeSp then
		Atk:SkillFail("\#W[战斗讯息]：魔法不足\#G\%\#W，无法施展天崩地裂");
		do return end
	end
	...
	if 1.31 * Atk:GetATK() > Target:GetDEF() then
	...
	local CritHit = {0, 0, 0};
	local DamRate = {0.95, 1.05, 1.1};
	local AtkNum = 3
	local CritRate = 5
	if Atk:GetSkillLv(394) > 20 then
	    FinalDam = {0, 0, 0};
	    CritHit = {0, 0, 0};
	...
	while i <= AtkNum do
		...
		FinalDam[i] = (DamRate[i] * (Atk:GetATK() - FinalDef  + 300) * wDam - 200) * randomDam;
