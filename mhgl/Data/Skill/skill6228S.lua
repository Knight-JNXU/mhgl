...
function skill6228CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill6228Use(Atk, Target, battle, skillLv)
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if Atk:GetMob_id() == 30741 then
				Dammodify = 0.5;
				recmodify = 0.5;
			end						
			
			if math.random(1,100) <= CritRate then
				FinalDam = Dammodify * 0.65 * ((1.05 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam * critDam;
				CritHit = 1;
			else
				FinalDam = Dammodify * 0.65 * ((1.05 * Atk:GetATK() - FinalDef + 250) * wDam - 250) * randomDam;
			end
			...
			Atk:ChangeHp(recmodify * 0.1 * Atk:GetMaxHP())
