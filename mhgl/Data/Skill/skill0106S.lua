...
function skill0106CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill0106Use(Atk, Target, battle, skillLv)

	local consumeSp = 0.4 * skillLv + 10;
	
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展子母针", consumeSp);
	...
	if result == 2 then
		...
		Target:AddBuff(19, skillLv, 0, 0, turn, 100);
	end
	battle:PushReactionActor(Target);
