...
function skill24031CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此技能需鬼蛊灵蕴修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill24031Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：魔法不足\#G%d\#W，无法施展夺命咒", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		local FinalDam = (skillLv * 10+100)*2*(1-(count-1)*0.1) +Atk:GetSkillLv(378)*15+weaponDam;
		if skillLv < 120 then
		...
		FinalDam = ((skillLv * 10+100)*2*(1-(count-1)*0.1) +Atk:GetSkillLv(378)*15+weaponDam)*0.5;
		end
