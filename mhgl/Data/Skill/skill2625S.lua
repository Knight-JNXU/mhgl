...
function skill2625CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill2625Use(Atk, Target, battle, skillLv)
	...
	if Atk:GetSkillLv(394) > 0 then -- 法宝秒数+1
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展落雷符", consumeSp);
	...
	while targetlist[i] ~= nil do
		...
		if hpChange > 2000 then
		   hpChange = hpChange - 300
		elseif hpChange > 1000 then
		   hpChange = hpChange - 220
		elseif hpChange > 800 then
		   hpChange = hpChange - 150
		end
		if hpChange <= 1 then
			hpChange = 1;
		end
		
		if Atk:GetSkillLv(394) > 0 then --法宝加伤害
