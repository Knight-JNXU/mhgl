...
function skill22016CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
...
function skill22016Use(Atk, Target, battle, skillLv)
	...
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展翻江搅海", consumeSp);
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			local randomDam = 1.0 - math.random(100, 300) / 900;
			local weaponDam = 0.28 * (Atk:GetHIT() - Atk:GetAGI() * 1.7 - 6);
			local critDam = 2.0;
			local FinalDef = targetlist[i]:GetDEF();
			if 1.05 * Atk:GetATK() > targetlist[i]:GetDEF() then
			...
			if math.random(1,100) <= CritRate then
				FinalDam = ((1.15 * Atk:GetATK() - 0.8 * FinalDef + weaponDam + 210) * wDam - 180) * randomDam * critDam;
				CritHit = 1;
			else
				FinalDam = ((1.15 * Atk:GetATK() - 0.8 * FinalDef + weaponDam + 210) * wDam - 180) * randomDam;
			end
