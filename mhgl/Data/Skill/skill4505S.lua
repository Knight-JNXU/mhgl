...
function skill4505CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
end

function skill4505Use(Atk, Target, battle, skillLv)
	
	local sLv = Target:GetLevel();
	local consumeSp = 1.0 * sLv;
	local Msg = string.format("\#W[战斗讯息]：内力不足\#G%d\#W，无法施展大悲绝魂", consumeSp);
	...
	while i <= lianji do
		...
		if math.random(1,100) <= buffRate and not Target:IsBuffExist(1705) then
			Target:AddBuff(1705, sLv, 0, 0, 10, 100);
		end
		i = i + 1;
	end
