...
function skill9793CanUse(Atk, skillLv, reqSkillLv, reqCharLv)	
end

function skill9793Use(Atk, Target, battle, skillLv)
	...
	if skill == 1 then
	    maxTargetCount = math.floor((Lv - 20)/ 25) + 3;
	...
	while targetlist[i] ~= nil do
		...
		local hpChange = ((Atk:GetPOW() - targetlist[i]:GetPOW()) * 1.15 + (2.4 - 0.1 * count) * (Lv + 10) + 260 + nDamsBonus1 * Lv)  * nDam - 250;
		
		local hp = 0 ;
		local lv = targetlist[i]:GetLevel()
		local A = 0
		if lv <= 159 then
		  A = math.random(600,800)
		elseif lv <= 105 then
