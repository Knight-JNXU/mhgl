...
function skill6232CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
end

function skill6232Use(Atk, Target, battle, skillLv)
	...
	while targetlist[i] ~= nil do
		if targetlist[i]:GetIndex() ~= Atk:GetIndex() then			
			...
			if result == 2 then
				...
				local buffrate = math.random(1, 100)
				targetlist[i]:AddAttriChangeToLst(-1 * FinalDam, 0, 0, 0, LuaI.BATCMD_SKILL_HIT);
				if Atk:GetMob_id() == 30708 then
					if buffrate <= 30 then
						targetlist[i]:AddBuff(1002, (Lv + 10), 0, 0, 5, 100);				
					elseif buffrate <= 50 then
						targetlist[i]:AddBuff(1005, (Lv + 10), 0, 0, 5, 100);
					end
				end
			end
