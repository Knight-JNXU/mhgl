...
function skill2421CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	
	if skillLv < reqSkillLv then 
		local Msg = string.format("\#W[战斗讯息]：此招式需紫电青芒手修炼到\#G%d\#W级以上才能施展", reqSkillLv);
		Atk:SkillFail(Msg);
		do return end
...
function skill2421Use(Atk, Target, battle, skillLv)

	local consumeSp = 0.3 * skillLv + 10;
	if Atk:IsBuffExist(1702) then
		consumeSp = consumeSp * 2;
	end
	local Msg = string.format("\#W[战斗讯息]：內力不足\#G%d\#W，无法施展子母锁喉针", consumeSp);
	...
	if Target:IsBuffExist(1101) then
		Atk:SkillFail("\#W[战斗讯息]：目标已经中了子母锁喉针")
		do return end
	end
