...
function skill11001CanUse(Atk, skillLv, reqSkillLv, reqCharLv)
	...
	if Atk:GetHP() < reqHp then
		local Msg = "\#W生命不足\#G20\%\#W，无法施展牛屎遁";
		Atk:SkillFail(Msg);
		do return end
	elseif Atk:GetHP() < consumeHp then
		local Msg1 = string.format("\#W气血不足\#G%d\#W，无法施展牛屎遁", consumeHp);
		Atk:SkillFail(Msg1);
		do return end
	elseif Atk:GetSP() < consumeSp then
		local Msg2 = string.format("\#W魔法不足\#G%d\#W，无法施展牛屎遁", consumeSp);
		Atk:SkillFail(Msg2);
		do return end
	elseif Atk:HasParty() then
		local Msg = "\#W组队状态下无法施展牛屎遁";
		Atk:SkillFail(Msg);
		do return end
...
function skill11001Use(Atk, Target, battle, skillLv)
	...
	if math.random(1, 100) <= Rate then
		Atk:JumpMap(18, 29, 176);
	else
		Atk:SkillFail("\#W学艺不精，牛屎遁施展失败");
	end
