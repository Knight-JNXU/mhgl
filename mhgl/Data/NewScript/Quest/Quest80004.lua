...
Quest80004 = {							   
	...
	target_item = {{item_id = 60007, count = 1}},	
	time_mark = 800, 
}

function Quest80004:Take()	
	...
	if player:GetGuildPos() ~= 1 and player:GetGuildPos() ~= 2 and player:GetGuildPos() ~= 7 and player:GetGuildPos() ~= 9 then
		NpcSay("只有帮主、副帮主和商人、精英才可以领取跑商任务。")
		return false
	end
	...
	if Fund < 40000 then
		NpcSay("帮派资金不足40000，无法领取跑商任务。")
		return false
	end		
	if player:GetQuests():TimeMarkOver(800, 60) == false then
	...
	if IsProcess() then		
		...
		player:GetItems():AddItem(60006, 1)
		guild:ChangeFund(-40000,6)		
		self.descript = "这是一张价值\#P40000\#W两的帮派商票，通过与各地商人的贸易往来，帮派希望你能让它的价值增加到\#P180000\#W两。"
