...
Quest30601 = {
	...
	time_mark = 306,                          -- 时间标志  
	encount_npc = {npc_id = 300281, map_id = 0, step = math.random(300,500), count = -1},
	target_follower = { mob_id = 999 },
	require = {minLv = 30}
}

-- Quest30601 拿取任务方法
function Quest30601:Take()
...
function Quest30601:Return()
	...
	script:SetSaying(true)
		...
		player:ChangeSMoney(smoney,306)
		player:ChangeMoney(money,306)
		RemoveQuest(self)
		player:GetItems():RemoveItem(70121,1)
		...
		if math.random(1,200) <= 20 and player:GetItems():GetFreeSlot(0) > 0 and qtype:GetTotalRound() < 1 then
			local item = RandTable({51300,51301,51302,51304,51305,51306,51307,51308,51310,51311})
