...
Npc350022 = {							
	npc_id = 350022,					
	name = GetNpcName(350022),	
}

function Npc350022:Talk(bProcess)	
	if player:GetQuests():HasQuest(60220) and npc and npc:GetNpc_id() == player:GetQuests():GetFlag(60220) then	
		--时间判定
		local flag60222 = player:GetQuests():GetFlag(60222)		
		local flag60223 = player:GetQuests():GetFlag(60223)	
		if flag60222 == 1 then
		    if player:GetQuests():TimeMarkOver(60221, 1) == false then
		        NpcSay("你被剥夺了1分钟的自由")
		        return 
		    else 
				if IsProcess() then
				    player:GetQuests():SetFlag(60222,0)
				    player:GetQuests():RemoveTimeMark(60221)
			    end
			end
		end
		if flag60223 == 1 then	
			if player:GetQuests():TimeMarkOver(60221, 2) == false then
			    NpcSay("你被剥夺了2分钟的自由")
			    return 
			else 
				if IsProcess() then
				    player:GetQuests():SetFlag(60223,0)
				    player:GetQuests():RemoveTimeMark(60221)
			    end
			end
		end
		...
		if flag60221 == 0 then
			...
			if p <= 1 then				    	
				NpcSay("\#W.......")
				ISay("有啥事就直说，别婆婆妈妈的。")
				NpcSay("\#W你刚花了\#P40000\#W银两买了一个戒子吧。")
				ISay("刚遇到一个颇有好感的人，所以就买了个戒子相赠，花这点小钱不算啥。")
				NpcSay("我跟你说，那个人其实是个...人妖...")
				ISay("你说啥......")
				NpcSay("算了，当我没说......")		
				player:ChangeMoney(-40000)				
				player:GetQuests():SetFlag(60221,1)		
				return				
			elseif p <= 2 then			    	
				NpcSay("\#W这位古民，你看你身上是不是少了啥东西。")
				ISay("......没啊......")
				NpcSay("\#W我刚看到有个人在你的钱包外面摸来摸去的......")
				ISay("\#W我×××，丢了\#P50000\#W银两。")						
				player:ChangeMoney(-50000)
				player:GetQuests():SetFlag(60221,1)		
				return							
			elseif p <= 3 then
			    NpcSay("\#W你运镖有功，特奖励你\#P2000\#W银票，以资鼓励。")
				player:ChangeSMoney(2000)	
				ShowError("获得银票2000")
				player:GetQuests():SetFlag(60221,1)
				return
			elseif p <= 4 then
			    NpcSay("\#W据我们庄主说，你举报外挂有功，特奖励你\#P50000\#W银票，以资鼓励。")				
				player:ChangeSMoney(50000)	
				ShowError("获得银票50000")
				player:GetQuests():SetFlag(60221,1)
				return			
			elseif p <= 5 then
				NpcSay("\#W身为一名古民，你竟然在公开场合乱扔垃圾物品，罚你\#P10000\#W银两，以示警告，下不为例！")
				if player:GetMoney() >= 10000 then					
					ISay("......那么隐秘的动作都让他看见了，我也只好破财消灾了......")
					player:ChangeMoney(-10000)
					player:GetQuests():SetFlag(60221,1)
					return
				else					
					ISay("城管兄弟，今儿个我出门没带钱包，你看......")
					NpcSay("\#W咦咦咦，想怎么滴，别以为你不带现金我就没法儿治你了，给我站这，直到逮着下一个扔垃圾的古民为止。")
					ISay("......")
					player:GetQuests():SetTimeMark(60221)
					player:GetQuests():SetFlag(60222,1)
					player:GetQuests():SetFlag(60221,1)
					player:AddBuff(261, 0, 0, 0, 1, 100)
                    return
				end
			elseif p <= 6 then
				NpcSay("\#W罚款\#P20000\#W银两。")
				ISay("......发生什么事了？")
				NpcSay("\#W有人揭发你虐待同伴。")
				if player:GetMoney() >= 20000 then					
					ISay("不就是把他弄死了几次，这也叫虐待......唉，认了。")					
					player:ChangeMoney(-20000)
					player:GetQuests():SetFlag(60221,1)
					return					
				else					
					ISay("没有的事啊，即便是到了喝豆浆吃大饼的地步，我也没让他少吃一口啊......")
					NpcSay("\#W你这种人我见多了，不老实交代就等着揭发你的人来了和你当面对峙。")
					ISay("......")
					player:GetQuests():SetTimeMark(60221)
					player:GetQuests():SetFlag(60223,1)
					player:GetQuests():SetFlag(60221,1)
					player:AddBuff(261, 0, 0, 0, 2, 100)
					return
				end			
			elseif p <= 7 then
			    NpcSay("\#W据说，你育新有功，特奖励你\#P200\#W点侠义值，以资鼓励。")				
				player:ChangeGoodness(200)
				ShowError("获得侠义值200")
				player:GetQuests():SetFlag(60221,1)
				return
			elseif p <= 8 then
			    ...
			    if freeSlot > 0 then
				    NpcSay("\#W据说，你捉鬼有功，特奖励你一个金柳露，以资鼓励。")
					player:GetItems():AddItem(30000, 1)	
					ShowError("获得金柳露")
					player:GetQuests():SetFlag(60221,1)
					return
				else
				    NpcSay("\#W本想奖励你的，可惜你的包裹已经满了，那就下次吧。")
					return
                end				
			end		
