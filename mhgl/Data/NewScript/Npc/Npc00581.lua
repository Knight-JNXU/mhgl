  ...
  if HasQuest(quest_ids)  then 
	...
	if GetAnswer() == 1 then
		...
		if GetAnswer() == 1 then
		  ...
		  if (job == 3 and sex == 0) or (job == 4 and sex == 0) then 
		    
		    NpcSay("很好,现在你已是化生寺弟子,努力将本门派发扬光大吧")
			 player:AddSkill(19000)
		    player:SetFaction(6)
		  else
		    NpcSay("本门只收人族男性弟子,你还是请回吧")
		  end
		end
	...
	elseif GetAnswer() == 2 then
	  if player:GetSkillLv(19000) < 1 then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	 if bProcess then
		 script:OpenInterface("skillshop", 10)
	  end
	elseif GetAnswer() == 3 then
		if player:GetFaction() == 6 then
