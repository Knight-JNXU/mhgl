...
Npc350026 = {							
	npc_id = 350026,					
	name = GetNpcName(350026),	
}

function Npc350026:Talk(bProcess)	
	if player:GetQuests():HasQuest(60220) and npc and npc:GetNpc_id() == player:GetQuests():GetFlag(60220) then	
		NpcAsk("\#W并不是每个商人都像我这样的，所以说出门在外多带点现金是没坏处的，下列商品挑一个吧。")				
		local price1 = math.random(80000,120000)        --高级马车票
		local price2 = math.random(80000,160000)      --白铁  
		local price3 = math.random(200000,400000)     --绿铁
		local price4 = math.random(300000,500000)     --紫铁合成碎石
		local price5 = math.random(500000,1000000)   --蓝铁精英   
		
		if math.random(1,100) <= 50 then
		    SetAnswer(1, string.format("高级藏宝图（%d银两）", price1))
		else
		    SetAnswer(2, string.format("超级金柳露（%d银两）", price2))
		end
		
		if math.random(1,100) <= 50 then
		    SetAnswer(3, string.format("四神石（%d银两）", price3))
		else
		    SetAnswer(4, string.format("90级百炼精铁（%d银两）", price4))
		end
		
		if math.random(1,100) <= 5 then
		    SetAnswer(5, string.format("100级百炼精铁（%d银两）", price5))		
		end		
		...
		if GetAnswer() == 1 then
			...
			if freeSlot > 0 then
				if player:GetMoney() >= price1 then								
					if not IsProcess() then		
						player:ChangeMoney(-price1)
						player:GetItems():AddItem(20001, 1)	--高级马车票	                      					
					end	
					ShowError("获得高级马车票")	
					NpcSay(string.format("\#W您花费了\#P%d\#W银两购得高级藏宝图。",price1))													
				else
					NpcSay("\#W您的现金不足，无法购物。")
				end
			else
			    ShowHint("您的包裹已满，无法继续购物。")
		    end	
		elseif GetAnswer() == 2 then
			local freeSlot = player:GetItems():GetFreeSlot(0)
			if freeSlot > 0 then
				if player:GetMoney() >= price2 then								
					if not IsProcess() then				    						
						player:ChangeMoney(-price2)
						player:GetItems():AddItem(30001, 1)		--白铁  								    
					end	
					ShowError("获得白铁精英")	
					NpcSay(string.format("\#W您花费了\#P%d\#W银两购得超级金柳露。",price2))
				else
					NpcSay("\#W您的现金不足，无法购物。")
				end
			else
		...
		elseif GetAnswer() == 3 then
			local freeSlot = player:GetItems():GetFreeSlot(0)
			if freeSlot > 0 then
				if player:GetMoney() >= price3 then								
					if not IsProcess() then				    							
						player:ChangeMoney(-price3)
						player:GetItems():AddItem(15007, 1)		--绿铁									    
					end	
					ShowError("获得绿铁精英")	
					NpcSay(string.format("\#W您花费了\#P%d\#W银两购得四神石。",price3))
				else
					NpcSay("\#W您的现金不足，无法购物。")
				end
			else
        ...
        elseif GetAnswer() == 4 then
			local freeSlot = player:GetItems():GetFreeSlot(0)
			if freeSlot > 0 then
				if player:GetMoney() >= price4 then								
					if not IsProcess() then				    							
						player:ChangeMoney(-price4)
						player:GetItems():AddItem(20254, 1)			--紫铁合成碎石								    
					end	
					ShowError("获得紫铁合成碎石")	
					NpcSay(string.format("\#W您花费了\#P%d\#W银两购得90级百炼精铁。",price4))
				else
					NpcSay("\#W您的现金不足，无法购物。")
				end
			else
        ...
        elseif GetAnswer() == 5 then
			local freeSlot = player:GetItems():GetFreeSlot(0)
			if freeSlot > 0 then
				if player:GetMoney() >= price5 then								
					if not IsProcess() then				    						
						player:ChangeMoney(-price5)
						player:GetItems():AddItem(20255, 1)				--蓝铁精英							    
					end	
					ShowError("获得蓝铁精英")	
					NpcSay(string.format("\#W您花费了\#P%d\#W银两购得100级百炼精铁。",price5))
				else
					NpcSay("\#W您的现金不足，无法购物。")
				end
			else
