...
--十二连环坞护法
Npc350153 = {npc_id = 350153, name = GetNpcName(350153)}
Npc350153.Talk = function(self, bProcess)
  ...
  if GetAnswer() == 1 then
		local quest63526 = player:GetQuests():GetQuest(63526)
		if player:GetLevel() < 40 then
			NpcSay("小朋友，还是练到\#G40\#W级再来挑战吧。")
			return false
		end
		if not quest63526 then
			NpcSay("请在戚继光处领取挑战任务，组上\#G5\#W人队伍，再根据任务提示逐一挑战。")
			return false
		end
    if q:GetTargetMobId(1) < 350153 then
      ...
      if q and q:GetTargetMobId(1) > 350153 then
