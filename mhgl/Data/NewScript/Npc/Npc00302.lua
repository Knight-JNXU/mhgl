...
function Npc00302:Talk(bProcess) 
  ...
  elseif GetAnswer() == 2 then
	...
	SetAnswer(1,"试试手气（70~80环装）10w/件")
	SetAnswer(2,"试试手气（50~60环装）10w/件")
	SetAnswer(3,"试试手气（50~80环装）7w/件")
	SetAnswer(4,"我只是路过")
    WaitAnswer(4)
	if GetAnswer() == 1 then
...
function GetRandomEq(bProcess,EqType)
   ...
   if EqType == 1 then
      if player:GetMoney() < 100000 then
        NpcSay("#A赌70-80环装每次需要10万银子，少侠的钱袋子好像空了喔！")
        return
      end
      ItemList7080 = {
			  {item_id = 20452},--金蛇信
   ...
   elseif EqType == 2 then
      if player:GetMoney() < 100000 then
        NpcSay("#A赌50-60环装每次需要10万银子，少侠的钱袋子好像空了喔！")
        return
      end
   ...
   elseif EqType == 3 then
      if player:GetMoney() < 70000 then
        NpcSay("#A赌50-80环装每次需要7万银子，少侠的钱袋子好像空了喔！")
        return
      end
      ItemList5080 = {
			  {item_id = 20452},--金蛇信
