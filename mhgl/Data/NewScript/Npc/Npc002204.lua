...
--十二连环坞护法
Npc002204 = {npc_id = 002204, name = GetNpcName(002204)}
Npc002204.Talk = function(self, bProcess)
  ...
  WaitAnswer(3)
  if GetAnswer() == 1 then
		local quest63527 = player:GetQuests():GetQuest(63527)
		if player:GetLevel() < 40 then
			NpcSay("小朋友，还是练到\#G40\#W级再来挑战吧。")
			return false
		end
		if not quest63527 then
			NpcSay("请在戚继光处领取挑战任务，组上\#G5\#W人队伍，再根据任务提示逐一挑战。")
			return false
		end
    if q:GetTargetMobId(1) < 915 then
      ...
      if q and q:GetTargetMobId(1) > 915 then
