...
Npc350034= {							
	npc_id = 350034,					
	name = GetNpcName(350034),	
}


function Npc350034:Talk(bProcess)	
	if npc and npc:GetNpc_id() == player:GetQuests():GetFlag(60220) then	
		...
		if flag60224 == 1 then	
			if player:GetQuests():TimeMarkOver(60221, 6) == false then
			    NpcSay("你被剥夺了6分钟的自由")
			    return 
			else 
				if IsProcess() then
		...
		if flag60221 == 0 then
			...
			if p <= 1 then
				NpcSay("\#W最新的运营数据显示你在游戏过程中有使用外挂的嫌疑，为了不影响其他玩家正常游戏，也为了给所有玩家一个公平的游戏环境，现决定剥夺你2分钟的自由。")							
				ISay("......")
				...
				return 
			elseif p <= 2 then
			    NpcSay("\#W刚收到古民们的联名上访信，信中说你经常诈骗一些新古民，这严重影响了古民们的口碑，为了净化古民们的生活环境，现决定剥夺你4分钟的自由。")							
				ISay("......")
				player:GetQuests():SetTimeMark(60221)
				...
				return 
			elseif p <= 3 then			
			    NpcSay("\#W城管们刚给我反应，说你到处乱贴代练信息，为了美化古民们的生活环境，现决定剥夺你6分钟的自由。")							
				ISay("......")
				player:GetQuests():SetTimeMark(60221)
