...
function Npc001000:Talk(bProcess)
  local quest_ids = {30601,30602,30603,30606,30605}
  NpcAsk({"菩萨让我化形再此等候有缘人，已经五百年了。",
          "自从我那侄儿沉香劈开华山，一有时间我就去看看我娘亲，就算神仙也不能抛弃血肉情！",
          "官人怎么现在才来呀~~",
 ...
 if player:GetFaction() == 0 then
  ...
  if HasQuest(quest_ids)  then 
		if player:GetQuests():HasQuest(30601) or player:GetQuests():HasQuest(30602) or player:GetQuests():HasQuest(30603) or player:GetQuests():HasQuest(30606) or player:GetQuests():HasQuest(30605) then
			SetAnswer(4, "跑镖任务")
		end
  end
	SetAnswer(8, "我什么都不想做")
	...
	if GetAnswer() == 1 then
		...
		if GetAnswer() == 1 then
		  ...
		  if (job == 1 and sex == 0) or (job == 2 and sex == 1) or (job == 3 and sex == 1) then 
		    NpcSay("很好,现在你已是我凌波城弟子,你可以在非战斗情况下使用\#P纵地金光\#W，让你安然返回这里。")
		    player:AddSkill(22000)
		    player:SetFaction(1)
		  else
		    NpcSay("本门只收仙族弟子,你还是请回吧")
			NpcSay("我对你没有眼缘,你还是请回吧")
		  end
		end
	elseif GetAnswer() == 4 then
			  ...
			  if bProcess then
			    script:SetSaying(true)
				  ...
				  or player:GetQuests():HasQuest(30606) and player:GetQuests():GetQuest(30606):IsFailed()
				  or player:GetQuests():HasQuest(30605) and player:GetQuests():GetQuest(30605):IsFailed()then
				    ...
				    if player:GetQuests():HasQuest(30601) and quest30601:GetEndNpcId() == self.npc_id then
					  ...
					  elseif player:GetQuests():HasQuest(30603) and quest30603:GetEndNpcId() == self.npc_id then
					    Quest30603:Return()
						 elseif player:GetQuests():HasQuest(30606) and quest30606:GetEndNpcId() == self.npc_id then
					    Quest30606:Return()
	...
	elseif GetAnswer() == 2 then
			  if player:GetSkillLv(22000) < 1 then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	 if bProcess then
		 script:OpenInterface("skillshop", 45)
	  end
	...
	elseif GetAnswer() == 4 then
		if player:GetFaction() == 1 then
			...
			if GetAnswer() == 1 then
				...
				--[[	elseif flag == 4 then
						if player:GetFDonate() > 983 then
							NpcSay("重置属性点成功。请慎重分配属性点，花费门贡重置属性点还有零次机会。")
							player:ResetCharLevel(lv)
							player:GetQuests():SetFlag(19,5)
							player:ChangeFDonate(-984)
						else
							NpcSay("门派贡献不足984点，无法重置属性点。")
							return
						end
					elseif flag == 5 then
						if player:GetFDonate() > 1463 then
							NpcSay("重置属性点成功。请慎重分配属性点，花费门贡重置属性点还有零次机会。")
							player:ResetCharLevel(lv)
							player:GetQuests():SetFlag(19,6)
							player:ChangeFDonate(-1464)
						else
							NpcSay("门派贡献不足1464点，无法重置属性点。")
							return
						end
					elseif flag == 6 then
						if player:GetFDonate() > 2039 then
							NpcSay("重置属性点成功。请慎重分配属性点，花费门贡重置属性点还有零次机会。")
							player:ResetCharLevel(lv)
							player:GetQuests():SetFlag(19,7)
							player:ChangeFDonate(-2040)
						else
							NpcSay("门派贡献不足2040点，无法重置属性点。")
							return
						end
					elseif flag == 7 then
						if player:GetFDonate() > 2711 then
							NpcSay("重置属性点成功。请慎重分配属性点，花费门贡重置属性点还有零次机会。")
							player:ResetCharLevel(lv)
							player:GetQuests():SetFlag(19,8)
							player:ChangeFDonate(-2712)
						else
							NpcSay("门派贡献不足2712点，无法重置属性点。")
							return
						end
					elseif flag == 8 then
						if player:GetFDonate() > 3479 then
							NpcSay("重置属性点成功。请慎重分配属性点，花费门贡重置属性点还有零次机会。")
							player:ResetCharLevel(lv)
							player:GetQuests():SetFlag(19,9)
							player:ChangeFDonate(-3480)
						else
							NpcSay("门派贡献不足3480点，无法重置属性点。")
							return
						end
					elseif flag == 9 then
						if player:GetFDonate() > 4343 then
							NpcSay("重置属性点成功。请慎重分配属性点，花费门贡重置属性点还有零次机会。")
							player:ResetCharLevel(lv)
							player:GetQuests():SetFlag(19,10)
							player:ChangeFDonate(-4344)
						else
							NpcSay("门派贡献不足4344点，无法重置属性点。")
							return
						end	]]
					else
