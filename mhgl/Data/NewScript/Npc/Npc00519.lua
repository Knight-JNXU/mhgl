...
function Npc00519:Talk(bProcess)
  local quest_ids = {30601,30602,30603,30604,30605}
  NpcAsk({"修行贵在用心领悟，切忌轻浮自满。",
        "龙族的法术玄妙精深，要苦心修习方能领悟。",
		"龙宫里有数不尽的宝贝，有机缘之人方能得到。",
		"要想成为仙界的精英，是要下一番苦功夫的。"})
  ...
  if HasQuest(quest_ids)  then 
	...
	if GetAnswer() == 1 then
		...
		if GetAnswer() == 1 then
		  ...
		  --and 必须同时满足
		  --or  满足一个条件
		  if (job == 2 and sex == 1) or (job == 3 and sex == 1 ) then
		    NpcSay("很好,现在你已是普陀山弟子,为师传你一招\#P坐莲\#W，方便你随时返回门派。")
		    player:AddSkill(20000)
		    player:SetFaction(5)
		  else
		    NpcSay("本门只收仙族女弟子,你还是请回吧")
		  end
		end
	...
	elseif GetAnswer() == 2 then
	  if player:GetSkillLv(20000) < 1 then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	  ...
	  if bProcess then
	    script:OpenInterface("skillshop", 11)
	  end
	elseif GetAnswer() == 3 then
		if player:GetFaction() == 5 then
