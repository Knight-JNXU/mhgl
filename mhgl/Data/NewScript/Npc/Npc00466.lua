...
Npc00466 = {							-- 杵官王
	npc_id = 466,						-- NPC ID
	name = GetNpcName(466),				-- NPC 名称
										-- NPC 会派出的任务表
}

-- Npc00019 对话方法，bProcess 为脚本处理阶段
function Npc00466:Talk(bProcess) 
  ...
  WaitAnswer(9)
	if GetAnswer() == 1 then
		...
		if GetAnswer() == 1 then
		  ...
		  if (job == 1 and sex == 0) or (job == 2 and sex == 1) or (job == 3 and sex == 1) then 
		    NpcSay("很好,现在你已是天宫弟子,努力将本门派发扬光大吧")
		    player:SetFaction(4)
	...
	elseif GetAnswer() == 6 then
	  if player:GetSkillLv(16000) < 1 then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	 if bProcess then
		 script:OpenInterface("skillshop", 7)
	  end
