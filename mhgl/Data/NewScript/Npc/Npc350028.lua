	...
	if player:GetQuests():HasQuest(60220) and npc and npc:GetNpc_id() == player:GetQuests():GetFlag(60220) then	       
		...
		if GetAnswer() == 1 then					
			if IsProcess() then	
                ...
                elseif p < 80 then
                    money = math.random(1,5000)		
                else
                    money = math.random(1,10000)
                end					
				player:ChangeSMoney(money,6022)
				ShowError(string.format("\#W获得银票%d", money))
				NpcSay(string.format("\#W恭喜你抽得\#P%d\#W银票",money))
			end			
		end      
		if IsProcess() then
			Quest60220:Take()
