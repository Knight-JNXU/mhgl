...
Npc00511 = {							-- 白晶晶
	npc_id = 511,						-- NPC ID
	name = GetNpcName(511),				-- NPC 名称
										-- NPC 会派出的任务表
}
-- Npc00019 对话方法，bProcess 为脚本处理阶段
function Npc00511:Talk(bProcess) 
	...
	if GetAnswer() == 11 then
		...
		if GetAnswer() == 1 then
		  ...
		  if (job == 1 and sex == 1) then
		    NpcSay("很好,现在你已是盘丝洞弟子,为师传你一招\#P移形换影\#W，方便你随时返回门派。")
		    player:AddSkill(15000)
		    player:SetFaction(8)
		  else
		    NpcSay("本门只收魔族女弟子,你还是请回吧")
		  end
		end
	elseif GetAnswer() == 6 then

	  if player:GetSkillLv(15000) < 1 then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	 if bProcess then
		 script:OpenInterface("skillshop", 6)
	  end
