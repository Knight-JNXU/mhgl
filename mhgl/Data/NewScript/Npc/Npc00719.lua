...
Npc00719.Talk = function(self, bProcess)
  ...
  elseif GetAnswer() == 3 then 
    NpcSay("附加的属性和强化功能只有50级以上的装备才拥有。\\r    装备的鉴定分为#R山寨、下品、中品、上品、极品、超极品#W6种，每次鉴定都会随机出现一种。首次进行鉴定只需要少量银两，再次鉴定则需要玩家持有对应的鉴定符。\\r    而关于装备强化，这个分为10个等级，每次强化需要消耗一个对应等级的强化精华，强化失败的话是会降低强化等级的噢~每个等级会降低的程度都不一样，请各位好好把握机会强化出最强装备吧！~")
  end
end
