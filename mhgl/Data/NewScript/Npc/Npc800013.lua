...
--十二连环坞护法
Npc800013 = {npc_id = 800013, name = GetNpcName(800013)}
Npc800013.Talk = function(self, bProcess)
	local q = player:GetQuests():GetQuest(63522) 
	NpcAsk("受皇帝之命,各大门派举行门派守护,江湖人士可进行闯关,胜利者可获得丰厚奖励！\r#P活动时间为每天晚间20:00-22:00\r#Y任务领取人:[长安门派闯关使者]")
	SetAnswer(1, "挑战")
	SetAnswer(2, "下一关")
  SetAnswer(3, "离开")
  WaitAnswer(3)
  if GetAnswer() == 2 then
   player:JumpMap(2020,80,250)
   end
  if GetAnswer() == 1 then
		local quest63522 = player:GetQuests():GetQuest(63522)
		if player:GetLevel() < 40 then
			NpcSay("小朋友，还是练到\#G40\#W级再来挑战吧。")
			return false
		end
		if not quest63522 then
			NpcSay("请在戚继光处领取挑战任务，组上\#G5\#W人队伍，再根据任务提示逐一挑战。")
			return false
		end
    if q:GetTargetMobId(1) < 800013 then
      ...
      if q and q:GetTargetMobId(1) > 800013 then
