...
Npc350066 = {							
	npc_id = 350066,					
	name = GetNpcName(350066),	
}

function Npc350066:Talk(bProcess)	
	if npc and npc:GetNpc_id() == player:GetQuests():GetFlag(60220) then	
		...
		if flag60223 == 1 then	
			if player:GetQuests():TimeMarkOver(60221, 4) == false then
			    NpcSay("你被剥夺了4分钟的自由")
			    return 
			else 
				if IsProcess() then
		...
		if flag60221 == 0 then
			...
			elseif p <= 2 then			    	
				...
				ISay("\#W我×××，丢了\#P5000\#W银两。")						
				player:ChangeMoney(-5000)
				player:GetQuests():SetFlag(60221,1)		
				return							
			elseif p <= 3 then
			    NpcSay("\#W据百里长青说，你运镖有功，特奖励你\#P2000\#W银票，以资鼓励。")
				player:ChangeSMoney(2000)	
				player:GetQuests():SetFlag(60221,1)
				return
			elseif p <= 4 then
			    NpcSay("\#W据我们庄主说，你举报外挂有功，特奖励你\#P5000\#W银票，以资鼓励。")				
				player:ChangeSMoney(5000)	
				player:GetQuests():SetFlag(60221,1)
				return			
			elseif p <= 5 then
				NpcSay("\#W身为一名古民，你竟然在公开场合乱扔垃圾物品，罚你\#P1000\#W银两，以示警告，下不为例！")
			...
			elseif p <= 7 then
			    NpcSay("\#W据魏子云说，你育新有功，特奖励你\#P200\#W点侠义值，以资鼓励。")				
				player:ChangeGoodness(200)
				player:GetQuests():SetFlag(60221,1)
				return
			elseif p <= 8 then
			    NpcSay("\#W据殷羡说，你大义灭青有功，特奖励你一块白铁，以资鼓励。")
				player:GetItems():AddItem(41400, 1)	
				player:GetQuests():SetFlag(60221,1)
				return
			end		
