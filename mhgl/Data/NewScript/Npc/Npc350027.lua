...
Npc350027 = {							
	npc_id = 350027,					
	name = GetNpcName(350027),	
}


function Npc350027:Talk(bProcess)	
	if player:GetQuests():HasQuest(60220) and npc and npc:GetNpc_id() == player:GetQuests():GetFlag(60220) then	
		...
		local flag60224 = player:GetQuests():GetFlag(60224)	
		if flag60222 == 1 then
		...
		if flag60224 == 1 then	
			if player:GetQuests():TimeMarkOver(60221, 3) == false then
			    NpcSay("你被剥夺了3分钟的自由")
			    return 
			else 
				if IsProcess() then
				    player:GetQuests():SetFlag(60224,0)
				    player:GetQuests():RemoveTimeMark(60221)
			    end
			end
		end		
		...
		if flag60221 == 0 then
			...
			if p <= 1 then
				NpcSay("\#W听说你刚才伤害了别的梅花桩，虽然我们只是木桩，可是也有尊严，你就在原地忏悔，减轻你的罪孽吧！你被限制行动1分钟！")							
				ISay("......")
				...
				return 
			elseif p <= 2 then
			    NpcSay("\#W好像刚才看到你踩到了掉落的梅花！我们庄主最爱这庄内的梅花了，你这么糟蹋，就好好处罚你一次！你被限制行动2分钟！")							
				ISay("......")
				player:GetQuests():SetTimeMark(60221)
				...
				return 
			elseif p <= 3 then			
			    NpcSay("\#W听说你认识陆小凤？那家伙经常来骚扰庄主，估计你来这里也没好事！你敢说不是？那就乖乖的呆着，让我们好好观察观察！你被限制行动3分钟！")							
				ISay("......")
				player:GetQuests():SetTimeMark(60221)
				player:GetQuests():SetFlag(60224,1)
				player:GetQuests():SetFlag(60221,1)
				player:AddBuff(261, 0, 0, 0, 3, 100)
				return 
			elseif p <= 4 then							
				if IsProcess() then			    					
					script:SetSaying(true)	
					local flag60225 = player:GetQuests():GetFlag(60225)
					local song = math.random(5,10)
					flag60225 = flag60225 + song
					player:GetQuests():SetFlag(60225,flag60225)		
					player:GetQuests():SetFlag(60221,1)
	                ShowError(string.format("\#R获得梅花桩积分%d", song))
					NpcSay(string.format("\#W我今儿个怎么觉得你和我特别的投缘？拿着，这是我送给你的\#P%d\#W点梅花桩积分。",song))				
				end		    		
			end			
