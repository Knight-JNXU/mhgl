...
Npc000401500 = {							
	npc_id = 401500,				--周婷仆人一
	name = GetNpcName(401500),	
}


function Npc000401500:Talk(bProcess)
	NpcAsk("就你也想打我神器的主意？哈哈~~~")
	SetAnswer(1, "找打~！")
	SetAnswer(2, "快跑")
	WaitAnswer(2)	
	if GetAnswer() == 1 then
		...
		for i = 0, party:GetRealMemNum() - 1 do
			...
			if member then					
				...
				if member:GetLevel() < 30 then
					NpcSayParty(string.format("\#P%s\#W等级不足\#G30\#W级，不可以玩这么危险的游戏的。", member:GetName()))
					return false
		...
		if bProcess then
			player:EncounterBattle(401500)
		end						
	end
	
end

AddNpcTable(Npc000401500)
