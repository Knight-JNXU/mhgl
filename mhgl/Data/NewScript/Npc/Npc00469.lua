...
Npc00531 = {							-- 杵官王
	npc_id = 531,						-- NPC ID
	name = GetNpcName(531),				-- NPC 名称
										-- NPC 会派出的任务表
}

-- Npc00019 对话方法，bProcess 为脚本处理阶段
function Npc00531:Talk(bProcess) 
 ...
 -- SetAnswer(4,"兑换斗战之书")
  ...
  if GetAnswer() == 11 then
      ...
      if player:GetFaction() ~= 0 then		 
        ...
        return
		  ...
		  if (job == 1 and sex == 1) or (job == 2 and sex == 0 ) then
		    NpcSay("很好,现在你已是地府弟子,为师传你一招\#P无常步\#W，方便你随时返回门派。")
	...
	elseif GetAnswer() == 6 then
	  if player:GetSkillLv(12000) < 1 then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	  if bProcess then
	    script:OpenInterface("skillshop", 3)
	  end
