...
--六扇门
Npc300500 = {npc_id = 300500, name = GetNpcName(300500)}
Npc300500.Talk = function(self, bProcess)
  ...
  --if getTime == "5" and hour >= 20 and hour < 23 then
  ...
  if GetAnswer() == 1 then
		...
		if not quest63521 then
			NpcSay("请在门派闯关使者处领取挑战任务，组上\#G5\#W人队伍，再根据任务提示逐一挑战。")
			return false
		end
    if q:GetTargetMobId(1) < 466 then
      ...
      if q:GetTargetMobId(1) > 466 then
		...
		for i = 0, party:GetRealMemNum() - 1 do  
		  ...
		  if member then		
		    if not q then
					NpcSayParty(string.format("\#P%s\#W身上没有挑战任务，请去门派闯关使者处领取。", member:GetName()))
					return false
...
--[[		if not party or party:GetRealMemNum() < 1 then
    ...
    local member = party:GetInPartyMember(i)
      if member then
        if not q then
          NpcSayParty(string.format("\#P%s\#W身上没有门派闯关任务，请去门派闯关使者处领取。", member:GetName()))
          return false
        end
