...
Npc00510.Talk = function(l_1_0, l_1_1)
 ...
 --SetAnswer(1, "膜拜")
    ...
    WaitAnswer(5)
    if GetAnswer() == 1 then
        if TotalRound6110 >= 200 then
          ShowHint("每天只能选一人膜拜一次")
          return
        end
      if player:GetLevel() < 10 then
      ShowHint("110级以下的战五渣，我不接受你的膜拜")
      return 
    end
        if l_1_1 then
          if type6110 == nil then
          ...
          --发奖励
	        --player:GetItems():AddItem(401680, 25)
	       -- player:GetItems():AddItem(80092, 1)					
	       -- player:GetItems():AddItem(80091, 3)				
           -- player:ChangeGDonate(90000)			
			 player:ChangeExp(1000000500)
			  end
			          NpcSay("领取成功")
		end
    if GetAnswer() == 2 then
