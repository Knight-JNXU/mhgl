  ...
  if answer == 1 then
    ...
    if not party then
      ...
      for i = 0, party:GetCount() - 1 do
        ...
        local qtype = member:GetQuests():GetType(407006)
        local TotalRound407006 = 0
        if qtype ~= nil then
          TotalRound407006 = qtype:GetTotalRound()
        end
        if qtype == nil or TotalRound407006 < 1 then
          if member:GetLevel() >= 69 then
	   if player:GetLevel() < 69 or player:HasTitle(1017) == false then
		ShowHint("看我嘴形“滚-滚-滚”")
		return false
		end	
          else
...
if l_1_1 then
  for i = 0, party:GetCount() - 1 do
    local member = party:GetMember(i)
    local type407006 = member:GetQuests():GetType(407006)
    if qtype == nil then
      member:GetQuests():SetRoundInfo(407006, 1, 1, 1)
      type407006 = member:GetQuests():GetType(407006)
      type407006:ChangeTotalRound(1)
    else
      type407006:ChangeTotalRound(1)
    end
  end
  player:EncounterBattle(407006)
end
   end
end
