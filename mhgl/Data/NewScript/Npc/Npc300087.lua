...
Npc300087 = {							-- 龙舟接引人2
	npc_id = 300087,						-- NPC ID
	name = GetNpcName(300087),				-- NPC 名称
										-- NPC 会派出的任务表
}

-- Npc00019 对话方法，bProcess 为脚本处理阶段
function Npc300087:Talk(bProcess)
	if npc then
		NpcAsk("欢迎参加中国梦幻古龙游泳大赛，我是2号接引人。")
		SetAnswer(1,"开始挑战")
	...
	--[[		if not ( Day >= 19 and Day <= 23 ) or not ( ( Hour >= 10 and Hour < 12 ) or ( Hour >= 15 and Hour < 17 ) or ( Hour >= 19 and Hour < 23 ) ) then
			...
			if player:GetItems():GetFreeSlot(0) ==0 then
				NpcSayParty("请队长身上至少留有一个空位。")
				return
			end
			if  bProcess then
				script:SetSaying(true)
				NpcSayParty("欢迎参加游泳大赛，现在开始比赛的第二关。")
				NpcSayParty("仲夏又至，这时候到江边吹下风必然是一件十分怡人的妙事，如果再放个小小的荷灯，那更是妙得紧哪！这是\#B荷灯\#W，你们就拿着这个到东海湾江边吹吹风，放荷灯吧。")
				player:GetItems():AddItem(80087)
				ShowError("\#W获得物品荷灯")
				Quest69203:Take()
			end
		end
