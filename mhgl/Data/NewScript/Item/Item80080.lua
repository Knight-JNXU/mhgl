...
function Item80080:MapUse(target)
	if target and target:GetUserType() == 2 then --同伴
		if target:GetLock() then 
			ShowHint("已上锁的同伴无法使用")
			return
		end	
		IAsk(string.format("你确定对\#P%s\#W使用吸附石？\r#Y请注意:使用后召唤兽会消失\r召唤兽没有特定技能也会消失~！\r#O本次使用的吸附石附加技能为【嗜血追击】", target:GetName()))
		SetAnswer(1, "确定")
		SetAnswer(2, "取消")			
		WaitAnswer(2)
		if GetAnswer() == 1 then		
		  ...
		  if not item:ReduceCount() then return end	
			...
			if target:HasSkill(NewSkillID) then
			  if i < 11 then 
			    ISay("吸附成功,获得嗜血追击魔兽要诀。")
			    player:GetPartners():RemovePartner(target)
			    player:GetItems():AddItem(40045,1)
...
function Item80080:CanUse(user, target)
end

function Item80080:BattleUse(user, target)

end

AddItemTable(Item80080)