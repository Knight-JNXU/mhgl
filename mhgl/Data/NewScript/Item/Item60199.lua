...
function Item60200:MapUse(target)
	if target and target:GetUserType() == 2 then --召唤兽
		...
		WaitAnswer(2)
		if GetAnswer() == 1 then				
			local NewSkillID = 9779
			local NewSkillName = "须弥真言"
			if target:HasSkill(NewSkillID) then
				...
				if skillcount <= 3 then	-- 召唤兽身上没有技能时直接给它新加技能
					target:AddSkill(NewSkillID)
					ShowHint(string.format("恭喜你的召唤兽学会了新的招式\#P%s", NewSkillName))
				elseif target:GetRace()~= 4 and skillcount <= 7 then
					local r = math.random(1, 100)
					if r <= 10 then	-- 30%直接学会新的技能
						target:AddSkill(NewSkillID)
						ShowHint(string.format("恭喜你的召唤兽学会了新的招式\#P%s", NewSkillName))
...
function Item60200:CanUse(user, target)
end

function Item60200:BattleUse(user, target)

end

AddItemTable(Item60200)