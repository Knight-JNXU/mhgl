...
function Item51386:MapUse(target)
	if target and target:GetUserType() == 2 then --同伴
		...
		WaitAnswer(2)
		if GetAnswer() == 1 then			
			local NewSkillID = 10003
			local NewSkillName = "四海升平"
			if target:HasSkill(NewSkillID) then
				...
				elseif target:GetRace()~= 7 and skillcount < 6 then
				...
				elseif target:GetRace() == 7 and skillcount < 7 then
					local r = math.random(1, 10)
					if r <= 7 then	-- 30%直接学会新的技能
						target:AddSkill(NewSkillID)
						ShowHint(string.format("恭喜你的同伴学会了新的招式\#P%s", NewSkillName))
...
function Item51386:CanUse(user, target)
end

function Item51386:BattleUse(user, target)

end

AddItemTable(Item51386)