--author: apollo
--武学秘籍 指定同伴后右击使用：使同伴学会"太极心法"招式

Item51107 = {
}

function Item51107:MapUse(target)
	if target and target:GetUserType() == 2 then --同伴
		...
		WaitAnswer(2)
		if GetAnswer() == 1 then				
			local NewSkillID = 257
			local NewSkillName = "太极心法"
			if target:HasSkill(NewSkillID) then
				...
				elseif target:GetRace()~= 4 and skillcount <= 6 then
...
function Item51107:CanUse(user, target)
end

function Item51107:BattleUse(user, target)

end

AddItemTable(Item51107)