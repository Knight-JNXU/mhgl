...
function Item60215:MapUse(target)
	if target and target:GetUserType() == 2 then --召唤兽
		...
		WaitAnswer(2)
		if GetAnswer() == 1 then				
			local NewSkillID = 291
			local NewSkillName = "高级隐身"
			if target:HasSkill(NewSkillID) then
				...
				elseif target:GetRace()~= 4 and skillcount < 3 then
				...
				elseif target:GetRace() == 4 and skillcount < 4 then
					local r = math.random(1, 10)
					if r <= 3 then	-- 30%直接学会新的技能
						target:AddSkill(NewSkillID)
						ShowHint(string.format("恭喜你的召唤兽学会了新的招式\#P%s", NewSkillName))
...
function Item60215:CanUse(user, target)
end

function Item60215:BattleUse(user, target)

end

AddItemTable(Item60215)