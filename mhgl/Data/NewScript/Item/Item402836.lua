  ...
  if GetAnswer() == 1 then
    ...
    if A ~= 0 then
      ...
      if GetAnswer() == 1 then
        ...
        IAsk("【票号掌柜】:少侠,本张元宝票可以充值150000元宝,请您签下您的大名，确定使用。")
        SetAnswer(1, "签名并确定使用")
        SetAnswer(2, "取消")
        WaitAnswer(2)
        if GetAnswer() == 1 then
          player:GetQuests():SetFlag(61024, 150000)
          player:GetItems():RemoveItem(402836, 1)
          ShowError("#W充值元宝150000个 1分钟内到账 元宝商城刷新即可")
        end
      end
      return
    end
