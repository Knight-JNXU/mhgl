...
Item81040 = {}
Item81040.MapUse = function(l_1_0, l_1_1)
  if l_1_1 and l_1_1:GetUserType() == 2 then
    ...
    ItemAsk(string.format("你确定要让#P%s#W使用成长丹吗？使用后请整理下背包", l_1_1:GetName()))
    SetAnswer(1, "确定")
    SetAnswer(2, "取消")
    WaitAnswer(2)
    if GetAnswer() == 1 then
      if l_1_1:GetGrow() >= 2.300 then
        ShowHint("同伴的成长需要在2300以下才能使用")
        return 
      end
      if not item:ReduceCount() then
        return 
      end
      l_1_1:ChangeGrow(0.005)
      if r > 0 then
...
Item81040.CanUse = function(l_2_0, l_2_1, l_2_2)
end

Item81040.BattleUse = function(l_3_0, l_3_1, l_3_2)
end

AddItemTable(Item81040)
