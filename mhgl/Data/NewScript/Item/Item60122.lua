...
function Item60122:MapUse(target)
	...
	if GetAnswer() == 1 then
		...
		if player:HasBuff(303) then
			...
			if not item:ReduceCount() then return end			
			player:AddBuff(303, 0, 0, 0, 60, 100)
			if type655 == nil then
				player:GetQuests():SetRoundInfo(655, 1, 14, 2);
				type655 = player:GetQuests():GetType(655);
				type655:ChangeTotalRound(1)	
...
function Item60122:CanUse(user, target)
end

function Item60122:BattleUse(user, target)

end

AddItemTable(Item60122)