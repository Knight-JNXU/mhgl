...
Npc00460 = {							-- 杵官王
	npc_id = 460,						-- NPC ID
	name = GetNpcName(460),				-- NPC 名称
										-- NPC 会派出的任务表
}

-- Npc00019 对话方法，bProcess 为脚本处理阶段
function Npc00460:Talk(bProcess) 
  local quest_ids = {30601,30602,30603}
  ...
  if HasQuest(quest_ids)  then 
		if player:GetQuests():HasQuest(30601) or player:GetQuests():HasQuest(30602) or player:GetQuests():HasQuest(30603) then
			SetAnswer(10, "跑镖任务")
		end
  end
  SetAnswer(9,"我什么都不想做")
  WaitAnswer(9)
	if GetAnswer() == 1 then
		...
		if GetAnswer() == 1 then
		  ...
		  if (job == 4 and sex == 1) then 
		    NpcSay("很好,现在你已是女儿村弟子,努力将本门派发扬光大吧")
		    player:SetFaction(5)
		  else
		    NpcSay("本门只收人族女弟子,你还是请回吧")
		  end
		end
	elseif GetAnswer() == 6 then
	  if player:GetFaction() ~= 5 and (job ~= 4 and sex ~= 1) then
		  NpcSay("本门武学从不外传。")
		  return
		end
	 if bProcess then
		 script:OpenInterface("skillshop", 8)
	  end
	elseif GetAnswer() == 10 then
		...
		if bProcess then
		 ...
		 or player:GetQuests():HasQuest(30603) and player:GetQuests():GetQuest(30603):IsFailed() then
			NpcSay("我的镖银呢？")
		else
