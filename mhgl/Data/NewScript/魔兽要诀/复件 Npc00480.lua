...
Npc00480 = {							-- 杵官王
	npc_id = 480,						-- NPC ID
	name = GetNpcName(480),				-- NPC 名称
										-- NPC 会派出的任务表
}

-- Npc00019 对话方法，bProcess 为脚本处理阶段
function Npc00480:Talk(bProcess) 
  local quest_ids = {30601,30602,30603,30604,30605}
  NpcAsk("要想在魔界扬名，还要多下苦功夫才行。!")
  ...
  if GetAnswer() == 11 then
	  ...
	  if GetAnswer() == 1 then
		  ...
		  if (job == 2 and sex == 0 ) then
		    NpcSay("很好,现在你已是狮驼岭弟子,为师传你一招\#P大鹏展翅\#W，方便你随时返回门派。")
		    player:AddSkill(1800)
		    player:SetFaction(2)
		  else
		    NpcSay("本门只收魔族男弟子,你还是请回吧")
		  end
		end
  ...
  elseif GetAnswer() == 4 then 
	  ...
	  local job = player:GetJob()
		local sex = player:GetSex()
		if (job ~= 2 or sex ~= 0) then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	  if player:GetSkillLv(1800) < 1 then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	  if bProcess then
	    script:OpenInterface("skillshop", 5)
	  end
