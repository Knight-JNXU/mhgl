...
Npc00539 = {							-- 杵官王
	npc_id = 539,						-- NPC ID
	name = GetNpcName(539),				-- NPC 名称
										-- NPC 会派出的任务表
}

-- Npc00019 对话方法，bProcess 为脚本处理阶段
function Npc00539:Talk(bProcess) 
  ...
  WaitAnswer(9)
	if GetAnswer() == 1 then
		...
		if GetAnswer() == 1 then
		  ...
		  if (job == 3 and sex == 0) or (job == 4 and sex == 1) or (job == 4 and sex == 0) then 
		    NpcSay("很好,现在你已是方寸山弟子,努力将本门派发扬光大吧")
		    player:SetFaction(3)
	...
	elseif GetAnswer() == 6 then
	  if player:GetFaction() ~= 3 then
		  NpcSay("本门武学从不外传。")
		  return
		end
	  local job = player:GetJob()
		local sex = player:GetSex()
		if (job ~= 3 or sex ~= 0) and (job ~= 4 or sex ~= 1) and (job ~= 4 or sex ~= 0)  then
		  NpcSay("本门武学从不外传。1")
		  return
	  end
	  if player:GetSkillLv(2600) < 1 then
		  NpcSay("本门武学从不外传。2")
		  return
	  end
	 if bProcess then
		 script:OpenInterface("skillshop", 9)
	  end
