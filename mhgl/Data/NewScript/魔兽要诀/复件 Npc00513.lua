...
Npc00513 = {							-- 杵官王
	npc_id = 513,						-- NPC ID
	name = GetNpcName(513),				-- NPC 名称
										-- NPC 会派出的任务表
}

-- Npc00019 对话方法，bProcess 为脚本处理阶段
function Npc00513:Talk(bProcess) 
  ...
  WaitAnswer(9)
	if GetAnswer() == 11 then
		...
		if GetAnswer() == 1 then
		  ...
		  if (job == 1 and sex == 0) then
		    NpcSay("很好,现在你已是五庄观弟子,为师传你一招\#P七星遁\#W，方便你随时返回门派。")
		    player:AddSkill(21000)
		    player:SetFaction(3)
		  else
		    NpcSay("本门只收仙族男弟子,你还是请回吧")
		  end
		end
	...
	elseif GetAnswer() == 6 then 
	  ...
	  local job = player:GetJob()
		...
		if (job ~= 1 or sex ~= 0) then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	  if player:GetSkillLv(21000) < 1 then
		  NpcSay("本门武学从不外传。")
		  return
	  end
	 if bProcess then
		 script:OpenInterface("skillshop", 12)
	  end
