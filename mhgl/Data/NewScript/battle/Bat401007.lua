...
function Bat000730:OnFinish(turn,result)
	...
	for i = 0, battle:GetCharacterCount() - 1 do
        ...
        elseif i < 20 then
          ...
          script:Rumor(string.format(" %s #O在挑战#Y地煞星#O时，打得对方四处奔逃，，只见他慌忙逃跑中抛出一个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
        elseif i < 30 then
          item = RandTable({80172})
				  name = GetItemName(item)
				  owner:GetItems():AddItem(item, 1)
					owner:GetScript():ShowError(string.format("\#W获得%s",name))
          script:Rumor(string.format(" %s #O在挑战#Y地煞星#O时，打得对方四处奔逃，，只见他慌忙逃跑中抛出一个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
        elseif i < 50 then
          item = RandTable({60052,60069,60085,60101,60117,60036,60311,20240,20224,20208,20192,20176,20160,20144,20128,20112,20096,20080})
				  name = GetItemName(item)
				  owner:GetItems():AddItem(item, 1)
					owner:GetScript():ShowError(string.format("\#W获得%s",name))
          script:Rumor(string.format(" %s #O在挑战#Y地煞星#O时，打得对方四处奔逃，，只见他慌忙逃跑中抛出一个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
        elseif i < 90 then
				  item = RandTable({60902,60906,60910,60914,60918,60922,60926,60930,60934,60938,60937,60933,60929,60925,60921,60917,60913,60909,60905,60901})--79级奖励
				  name = GetItemName(item)
				  owner:GetItems():AddItem(item, 1)
					owner:GetScript():ShowError(string.format("\#W获得%s",name))
          script:Rumor(string.format(" %s #O在挑战#Y地煞星#O时，打得对方四处奔逃，，只见他慌忙逃跑中抛出一个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
