...
Bat700046.OnInit = function(l_1_0)
  battle:AddMob(1311, 2,99)
  battle:AddMob(1301, 3,139)
  battle:AddMob(1309, 4,139)
  battle:AddMob(1309, 5,149)  
  battle:AddMob(1307, 6,149)   
   battle:AddMob(1306, 7,149) 
   battle:AddMob(1308, 8,159) 
  battle:AddMob(1308, 9,159) 
   battle:AddMob(1308, 1,159)  
end
  ...
  for i = 0, battle:GetCharacterCount() - 1 do
    ...
    if actor ~= nil then
      ...
      if owner and result == LuaI.END_WIN and not actor:IsDead() then
        owner:AddTitle(RandTable({1001}))
				owner:RemoveTitle(1000)
    ...
    if actor then
      ...
      if owner and not actor:IsDead() then
        ...
        if freeSlot > 1 then
          ...
          if SecondP <= 40 then
local ItemList = {61026,61027,61028,61029,61030,61031,61032,61033,15000,60333,60330,401643,80084,80091,80170,80171,80172,401670,401675,402831,402832,402830,402800,402801,60417,60427,60437,60447,60457,60467,60477,60487}
            ...
            local ItemList = {15006,15007,15008,15009,20001,30001,60416,60426,60436,60446,60456,60466,60476,60486,402800,402801,402830,402831}
            local ItemId = RandTable(ItemList)
            ShowMessage(string.format("#W获得%s", GetItemName(ItemId)), actor:GetOwner())
            script:Rumor(string.format("#W%s#O在#R水陆大会#O上英勇的消灭前来破坏的妖怪，得到举办方嘉奖#Y%s#O一个", owner:GetName(), GetItemName(ItemId)), false)
            owner:GetItems():AddItem(ItemId, 1)
