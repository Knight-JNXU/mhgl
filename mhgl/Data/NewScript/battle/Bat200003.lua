...
--[[	battle:DisableCmd(LuaI.BATCMD_HELP)
	...
	battle:AddMob(2001,1,155)
	battle:AddMob(2001,2,155)
	battle:AddMob(2001,3,155)
	battle:AddMob(2002,4,155)
	battle:AddMob(2002,5,155)
	battle:AddMob(2002,6,155)
	battle:AddMob(2005,7,155)
	battle:AddMob(2005,8,155)
	battle:AddMob(2006,9,155)
end
...
-- 回合开始之前
-- turn : 第几回合，从 1 开始
function Bat200003:OnPreTurn(turn)
...
-- 回合结束以后
-- turn : 第几回合，从 1 开始
function Bat200003:OnPostTurn(turn)	
...
-- turn : 结束回合
function Bat200003:OnFinish(turn,result)
	...
	for i = 0, battle:GetCharacterCount() - 1 do
		...
		if owner then
          ...
          script:Rumor(string.format("#A%s#P抓住铅笔小G刚要张嘴吃掉，吓的铅笔小G赶紧交出手里的#Y[%s]#P跪地求饶！", owner:GetName(), name), false)
		  else
            item = RandTable({600001})
				    name = GetItemName(item)
				    owner:GetItems():AddItem(item, 1)
					  owner:GetScript():ShowError(string.format("\#W获得%s",name))		-- 80书铁
					  owner:GetScript():ShowHint(string.format("#P获得#Y[%s]",name))
            script:Rumor(string.format("#A%s#P抓住铅笔小G刚要张嘴吃掉，吓的铅笔小G赶紧交出手里的#Y[%s]#P跪地求饶！", owner:GetName(), name), false)
          end
        elseif i <= 75 then
          item = RandTable({402802})
				  name = GetItemName(item)
				  owner:GetItems():AddItem(item, 1)
					owner:GetScript():ShowError(string.format("\#W获得%s",name))		--强化石
					owner:GetScript():ShowHint(string.format("#P获得#Y[%s]",name))
            script:Rumor(string.format("#A%s#P抓住铅笔小G刚要张嘴吃掉，吓的铅笔小G赶紧交出手里的#Y[%s]#P跪地求饶！", owner:GetName(), name), false)
        end
      end
