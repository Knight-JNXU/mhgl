...
Bat700046.OnInit = function(l_1_0)
  ...
  battle:AddMob(1400,2,base_lv)
  battle:AddMob(1401,3,base_lv)
  battle:AddMob(1401,4,base_lv)  
  battle:AddMob(1402,5,base_lv)   
   battle:AddMob(1402,6,base_lv)   
end
  ...
  for i = 0, battle:GetCharacterCount() - 1 do
    ...
    if actor ~= nil then
      ...
      if owner and result == LuaI.END_WIN and not actor:IsDead() then
        owner:AddTitle(RandTable({1010}))
        ShowMessage("#W战斗胜利", actor:GetOwner())
        local freeSlot = owner:GetItems():GetFreeSlot(0)
        if freeSlot > 0 then
		  owner:GetItems():AddItem(402910, 1)
          ShowMessage("#W获得邀请身份令牌", actor:GetOwner())
        end
      end
    end
  end
    ...
    if actor then
      ...
      if owner and not actor:IsDead() then
        local freeSlot = owner:GetItems():GetFreeSlot(0)
        if freeSlot > 1 then
          local SecondP = math.random(1, 150)
          if SecondP <= 10 then
