...
Bat000200004 = {npc_id = 200004,name = GetNpcName(200004),addMob = true,BossDead = false,kill = false,weizhi = 0,catch = false,r = math.random(1,16)}
function Bat000200004:OnInit()
...
function Bat000200004:OnPreTurn(turn)
  battle:DisableCmd(LuaI.BATCMD_HELP)
...
function Bat000200004:OnPostTurn(turn)
end
			  
			  
function Bat000200004:OnFinish(turn, result)
	    ...
	    local actor = battle:GetCharacter(i)
		  ...
		  if member then
				...
				local A = math.random(1,1500)
                 if A <= 1500 then
				 member:GetItems():AddItem(600001, 10)
				  member:GetItems():AddItem(555559, 1)
				   member:GetItems():RemoveItem(555558, 10)
        end
...
-- 必需在脚本最後将战斗表加入系统中
function Bat000200004:OnEscape(turn, actor)
end

AddBattleTable(Bat000200004)
