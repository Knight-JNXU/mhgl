...
Bat0002204 = {npc_id = 2204,name = GetNpcName(2204),addMob = true,BossDead = false,kill = false,weizhi = 0,catch = false,r = math.random(1,16)}
function Bat0002204:OnInit()
...
function Bat0002204:OnPreTurn(turn)
  battle:DisableCmd(LuaI.BATCMD_HELP)
...
function Bat0002204:OnPostTurn(turn)
end
			  
			  
function Bat0002204:OnFinish(turn, result)
	    ...
	    local actor = battle:GetCharacter(i)
		  ...
		  if member then
				...
				local A = math.random(1,1500)
                 if A <= 1500 then
				  member:GetItems():AddItem(402802, 1)
				  member:GetItems():AddItem(402803, 1)
        end
...
-- 必需在脚本最後将战斗表加入系统中
function Bat0002204:OnEscape(turn, actor)
end

AddBattleTable(Bat0002204)
