...
--[[	battle:DisableCmd(LuaI.BATCMD_HELP)
	...
	local base_lv = player:GetLevel() - 20
	local actorCount = battle:GetActorCount();	
	for i = 0, actorCount - 1 do
		local actor = battle:GetActor(i);							
	end
	battle:AddMob(1400, 1, base_lv)
	battle:AddMob(1401, 1, base_lv)
	battle:AddMob(1402, 1, base_lv)	
	battle:AddMob(1403, 1, base_lv)	
	battle:AddMob(1404, 1, base_lv)	
	battle:AddMob(1405, 1, base_lv)	
	battle:AddMob(1406, 1, base_lv)	
	battle:AddMob(958, 1, base_lv)	
	battle:AddMob(959, 1, base_lv)	
end
	...
	for i = 0, battle:GetCharacterCount() - 1 do
        ...
        if i < 5 then
          if a == 1 then
            ...
            script:Rumor(string.format(" %s #O在捉捕神兽的过程中特别卖力，童子奖历其一个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
          elseif a < 0 then
				    item = RandTable({80702})
				    name = GetItemName(item)
				    owner:GetItems():AddItem(item, 1)
					  owner:GetScript():ShowError(string.format("\#W获得%s",name))
            script:Rumor(string.format(" %s #O在捉捕神兽的过程中特别卖力，童子奖历其一个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
				...
				elseif i < 10 then
				  item = RandTable({60900,60904,60908,60912,60916,60920,60924,60928,60932,60936})
				  name = GetItemName(item)
				  owner:GetItems():AddItem(item, 1)
					owner:GetScript():ShowError(string.format("\#W获得%s",name))
          script:Rumor(string.format(" %s #O在捉捕神兽的过程中特别卖力，童子奖历其一个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
        ...
        item = RandTable({15006,15007,15008,15009})	  
				  name = GetItemName(item)
				  owner:GetItems():AddItem(item, 3)
					owner:GetScript():ShowError(string.format("\#W获得%s",name))
          script:Rumor(string.format(" %s #O在捉捕神兽的过程中特别卖力，童子奖历其1个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
        elseif i < 30 then
          item = RandTable({51900,51901,51902,51903,51904,51905,51906,51907,51908,51909,51910,51911,51912,51913,51914,51915,51916,51917,51918,51919,51920,51921,51922})
				  name = GetItemName(item)
				  owner:GetItems():AddItem(item, 1)
					owner:GetScript():ShowError(string.format("\#W获得%s",name))
          script:Rumor(string.format(" %s #O在捉捕神兽的过程中特别卖力，童子奖历其1个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
        elseif i < 100 then
           item = RandTable({30000})
                name = GetItemName(item)
				  owner:GetItems():AddItem(item, 1)
					owner:GetScript():ShowError(string.format("\#W获得%s",name))
          script:Rumor(string.format(" %s #O在捉捕神兽的过程中特别卖力，童子奖历其一个#Y[%s]#O！", owner:GetCharNameLink(), name), false)
