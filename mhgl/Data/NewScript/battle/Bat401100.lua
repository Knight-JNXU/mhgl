...
-- 暂为空表，但必需要定义
function Bat401100:OnInit()
battle:SetPunish(false)
end

-- 回合开始之前
-- turn : 第几回合，从 1 开始
function Bat401100:OnPreTurn(turn)
battle:DisableCmd(LuaI.BATCMD_HELP)
    if turn == 1 then
		local mob ={957,958,940,941,990,991,992,993,994,995,996,935,952,953,954,955,936,937}
		battle:AddMob(1110, 1)		
...
-- 回合结束以后
-- turn : 第几回合，从 1 开始
function Bat401100:OnPostTurn(turn)
end

-- 战斗结束之后
-- result : 玩家战斗结果, LuaI.END_WIN=玩家胜利，LuaI.END_LOSE=玩家战败，LuaI.END_DRAW=战斗平手
-- turn : 结束回合
function Bat401100:OnFinish(turn, result)
