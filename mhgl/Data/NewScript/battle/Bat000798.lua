...
-- 暂为空表，但必需要定义
function Bat000798:OnInit()
end

-- 回合开始之前
-- turn : 第几回合，从 1 开始
function Bat000798:OnPreTurn(turn)
    battle:DisableCmd(LuaI.BATCMD_HELP)
end

-- 回合结束以后
-- turn : 第几回合，从 1 开始
function Bat000798:OnPostTurn(turn)
end

-- 战斗结束之后
-- result : 玩家战斗结果, LuaI.END_WIN=玩家胜利，LuaI.END_LOSE=玩家战败，LuaI.END_DRAW=战斗平手
-- turn : 结束回合
function Bat000798:OnFinish(turn, result)
		...
		for i = 0, battle:GetCharacterCount() - 1 do
			...
			if actor ~= nil then
				if not actor:IsDead() and actor:GetOwner() then
					Quest65300:Return(actor:GetOwner())
				end
			end	
		end
	end
