	...
	for i = 0, battle:GetCharacterCount() - 1 do
		...
		if owner then
			if result == LuaI.END_WIN and not actor:IsDead() then
			    local QuestFlagC = owner:GetQuests():GetFlag(55203) -- 小仙计数 BB进阶专用
	            owner:GetQuests():SetFlag(55203, QuestFlagC + 1) -- 小仙计数 BB进阶专用
				...
				if LvPlayer > 90 then
				Money = math.random(500,1000)+LvPlayer*100
				else
				Money = math.random(500,1000)
				end
				Exp = math.floor(lvdiffModify *((4 * LvMob + 10) * 6.6 * 4) )* 1.3 * 50 * 2.3
