...
-- 暂为空表，但必需要定义
Preload("Quest304001.lua")

Bat401202 = {addMob = true,kill = false,weizhi = 0,catch = false,r = math.random(1, 3)
}

-- 战斗开始之前
function Bat401202:OnInit()
  local MobId = {1143,1144,1145,420,268,254,272,943,945,956,957,958,959,980,981,982,983}
  local i = 1
  local o = math.random(5,7)
  for i = 0, o do
...
-- 回合开始之前
-- turn : 第几回合，从 1 开始
function Bat401202:OnPreTurn(turn)
	-- 在第一回合开始的时候
...
-- 回合结束以后
-- turn : 第几回合，从 1 开始
function Bat401202:OnPostTurn(turn)
...
-- turn : 结束回合
function Bat401202:OnFinish(turn, result)
	if result == LuaI.END_WIN then
		for i = 1, battle:GetCharacterCount() - 1 do
            ...
            if actor:GetOwner() then
				if not actor:IsDead() then
					Quest304001:Return(actor:GetOwner(),partnerAlive)
				else
					Quest304001:PlayerWinButDied(actor:GetOwner())
				end
            end
		end
        ...
        if leader:GetOwner() then
			if not leader:IsDead() then
				Quest304001:leaderReturn(leader:GetOwner(),partnerAlive)
			else
				Quest304001:PlayerWinButDied(leader:GetOwner())
			end
		end
