...
-- 暂为空表，但必需要定义
function Bat00657:OnInit()
end

-- 回合开始之前
-- turn : 第几回合，从 1 开始
function Bat00657:OnPreTurn(turn)
...
-- 回合结束以后
-- turn : 第几回合，从 1 开始
function Bat00657:OnPostTurn(turn)
end

-- 战斗结束之后
-- result : 玩家战斗结果, LuaI.END_WIN=玩家胜利，LuaI.END_LOSE=玩家战败，LuaI.END_DRAW=战斗平手
-- turn : 结束回合
function Bat00657:OnFinish(turn, result)
...
function Bat00657:OnEscape(turn, actor)
	if battle:GetCharacterCount() == 0 and npc then
		npc:SetEnableBattle(true)
	end
end
-- 必需在脚本最後将战斗表加入系统中
AddBattleTable(Bat00657)
