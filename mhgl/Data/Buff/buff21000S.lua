...
function buff21000Active(buff, Char, Other, Type) 
	...
	if Type == DO_HITTING then
		...
		if Char:GetTeam() ~= Other:GetTeam() then
			...
			if (Other:GetAction() == LuaI.BATCMD_ATTACK or Other:GetAction() == LuaI.BATCMD_CRIT_ATTACK ) and 
				...
				local CritHit = 0;
				if math.random(1,100) <= 5 then
					FinalDam = 0.5 * ((Char:GetATK() - FinalDef + 250) * wDam - 250)  * randomDam * critDam;
					CritHit = 1;
				else
					FinalDam = 0.5 * ((Char:GetATK() - FinalDef + 250) * wDam - 250) * randomDam;
				end
