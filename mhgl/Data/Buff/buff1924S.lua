...
function buff1924Active(buff, Char, Other, Type) 
	...
	if Type == DO_HITTING_ATTACKER then
		...
		--		Other:ResetMHP();
				...
				if not Other:IsBuffExist(0) and not Other:IsBuffExist(2044) and not Other:IsBuffExist(0) and not Other:IsBuffExist(0) then
					...
					elseif Char:IsBuffExist(21043) then 
						CritRate = 25;
					elseif Char:IsBuffExist(1925) then
						CritRate = 55;
					end
				end
				...
				while i <= lianji do
					...
					if math.random(1,100) <= CritRate then
						FinalDam[i] = critDam * (((Char:GetATK() - FinalDef + 250) * wDam - 250) * randomDam) * 0.8;
						CritHit = 1;
					else
						FinalDam[i] = ((Char:GetATK() - FinalDef + 250) * wDam - 250) * randomDam  * 0.8;
					end
