...
function buff1524Active(buff, Char, Other, Type) 
	...
	if Type == DO_HITTING_ATTACKER then	
		...
		if (Char:GetAction() == LuaI.BATCMD_SKILL or Char:GetAction() == LuaI.BATCMD_CRIT_SKILL and 
			skillForm == 2) and (reDam1 > 0 or reDam2 > 0 or reDam3 > 0) and 
			Other:GetReaction() ~= LuaI.BATCMD_MISS then
			
			if reDam1 > 0 then
				Other:ResetHP();
				Other:ChangeHp(-1.075 * reDam1);
			end
			if reDam2 > 0 then
				Other:ResetSP();
				Other:ChangeSp(-1.075 * reDam2);
			end
			if reDam3 > 0 then
				Other:ResetMHP();
				Other:ChangeMHp(-1.075 * reDam3);
			end
		end		
	end	
end
