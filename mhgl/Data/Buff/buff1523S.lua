...
function buff1523Active(buff, Char, Other, Type) 
		...
		if (Other:GetAction() == LuaI.BATCMD_ATTACK or Other:GetAction() == LuaI.BATCMD_CRIT_ATTACK or
			...
			if reDam1 > 0 then	
				...
				if (Char:IsBuffExist(1102) or Char:IsBuffExist(1105)) and not Char:IsBuffExist(1410) then
					reDam1 = reDam1 * (1.025 + sectrate);
				elseif Char:IsBuffExist(1410) then
					reDam1 = reDam1 * (1 + sectrate);
				else	
					reDam1 = reDam1 * (0.925 + sectrate);
				end
			...
			if reDam2 > 0 then
				Char:ResetSP();				
				if (Char:IsBuffExist(1102) or Char:IsBuffExist(1105)) and not Char:IsBuffExist(1410) then
					reDam2 = reDam2 * (1.025 + sectrate);
				elseif Char:IsBuffExist(1410) then
					reDam2 = reDam2 * (1 + sectrate);
				else
					reDam2 = reDam2 * (0.925 + sectrate);
				end
			...
			if reDam3 > 0 then
				Char:ResetMHP();				
				if (Char:IsBuffExist(1102) or Char:IsBuffExist(1105)) and not Char:IsBuffExist(1410) then
					reDam3 = reDam3 * (1.025 + sectrate);
				elseif Char:IsBuffExist(1410) then
					reDam3 = reDam3 * (1 + sectrate);
				else
					reDam3 = reDam3 * (0.925 + sectrate);
				end
