...
function buff21019Active(buff, Char, Other, Type) 
	...
	if Type == DO_HITTING_ATTACKER then	
		...
		if (Char:GetAction() == LuaI.BATCMD_ATTACK or Char:GetAction() == LuaI.BATCMD_CRIT_ATTACK or 
			...
			if (Other:IsBuffExist(2020) or Other:IsBuffExist(2053)) then
			...
			elseif (Other:IsBuffExist(2021) or Other:IsBuffExist(2054)) then
				if reDam1 > 0 then
					Other:ResetHP();
