...
function buff1928Active(buff, Char, Other, Type) 
	...
	if Type == DO_HITTING then
		...
		if Char:GetTeam() ~= Other:GetTeam() then
			...
			if (Other:GetAction() == LuaI.BATCMD_ATTACK or Other:GetAction() == LuaI.BATCMD_CRIT_ATTACK ) and 
				...
				Other:IsBuffExist(0) ~= true and 
				Other:IsBuffExist(0) ~= true and
				Other:IsBuffExist(0) ~= true and 
				Other:IsBuffExist(0) ~= true then
				...
				if 0.85 * Char:GetATK() > Other:GetDEF() then
				...
				local CritHit = 0;
				if math.random(1,100) <= 5 then
					FinalDam = ((Char:GetATK() - FinalDef + 250) * wDam - 200)  * randomDam * critDam;
					CritHit = 1;
				else
					FinalDam = ((Char:GetATK() - FinalDef + 250) * wDam - 200) * randomDam;
				end
