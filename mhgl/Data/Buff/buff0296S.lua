--镇帮神器：剑侍扣血（最大值百分比） 
function buff0296Active(buff, Char, Other, Type) 
	
	if Type == TURN_START then
		if Char:GetHP() ~= 0 then
			local rec = Char:GetMaxHP() * 0.02
			if rec <= 1 then
				rec = 1;
			end
			...
			for i = 0, actorCount - 1 do
				...
				if (actor ~= nil and actor:IsPlayer()) then
					a = actor:GetOwner():GetQuests():GetNpcFlag(1004,NpcId,6)
					break
				end					
			end
			...
			if Char:GetHP() > 0 then
				...
				for t = 0, actorCount - 1 do
					...
					if (actor ~= nil and actor:IsPlayer()) then
						actor:GetOwner():GetQuests():SetNpcFlag(1004,NpcId,6,0)
						break
					end					
				end
			end
