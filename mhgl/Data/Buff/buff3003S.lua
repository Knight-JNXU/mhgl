...
function buff3003Active(buff, Char, Other, Type) 
	...
	if Type == DO_HITTING_ATTACKER then
		...
		if Char:GetTeam() ~= Other:GetTeam() then
			...
			if ((Char:GetAction() == LuaI.BATCMD_SKILL or 
				...
				if Dam1 ~= 0 then
					Other:ResetHP();
					Other:ChangeHp(1.0 * Dam1 - 2 * nDamsBonus * Lv);
				end
				if Dam2 ~= 0 then 
					Other:ResetSP();
					Other:ChangeSp(1.0 * Dam2 - 2 * nDamsBonus * Lv);
				end
				if Dam3 ~= 0 then 
					Other:ResetMHP();
					Other:ChangeMHp(1.0 * Dam3 - 2 * nDamsBonus * Lv);
				end
			end
		else 
			...
			if ((Char:GetAction() == LuaI.BATCMD_SKILL or 
				...
				if Dam1 ~= 0 then
					Other:ResetHP();
					Other:ChangeHp(1.0 * Dam1 + 2 * Lv);
				end
				if Dam2 ~= 0 then 
					Other:ResetSP();
					Other:ChangeSp(1.0 * Dam2 + 2 * Lv);
				end
				if Dam3 ~= 0 then 
					Other:ResetMHP();
					Other:ChangeMHp(1.0 * Dam3 + 2 * Lv);
				end
			end
		end
	end	
