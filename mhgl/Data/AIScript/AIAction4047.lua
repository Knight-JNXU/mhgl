function AIAction4047(battle, mob)	
	...
	for i = 0, actorCount - 1 do
		...
		--[[if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and 
			actor:GetIndex() == mob:GetIndex() and not actor:IsBuffExist()) then
	...
	if actorListCount > 0 then
		...
		elseif Rate > AtkRate and Rate <= SkillRate then --skill 70%
			...
			local csv = mob:GetSkillCsvData(randomSkill);
				if csv ~= nil then
	               	if csv.id == 5103 then
						if actorListCount1 > 0 and not mob:IsBuffExist(1811) then
