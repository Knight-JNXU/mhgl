function AIAction4010(battle, mob)	
	...
	for i = 0, actorCount - 1 do
		...
		--万毒萦绕目标选择列表
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and not actor:IsBuffExist(1002) and (not actor:HasSkill(5339) and not actor:HasSkill(5330))) then
			actorList1[actorListCount1] = actor:GetIndex();
			actorListCount1 = actorListCount1 + 1;
		end
		
		--回龙克敌术目标选择列表
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and actor:GetFaction() == 5 and actor:GetCclass() == 2) then
		...
		--碧水清茗目标选择列表
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and actor:IsPlayer() and not actor:IsBuffExist(1201)) then
	...
	if battle:GetCurTurn() == 1 then
		if actorListCount > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList[R], 5200, 0);
		else
	...
	elseif battle:GetCurTurn() == 2 then
		if protectListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, protectList1[T], 5202, 0);
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end			
	elseif battle:GetCurTurn() == 3 then
		if actorListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList1[S], 5204, 0);
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 4 then
		if protectListCount2 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, protectList2[U], 5201, 0);
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end		
	else
