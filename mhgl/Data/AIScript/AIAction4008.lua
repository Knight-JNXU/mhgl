function AIAction4008(battle, mob)	
	...
	for i = 0, actorCount - 1 do
		...
		--己方所有有效目标选择列表
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and actor:IsPlayer()) then
	...
	elseif battle:GetCurTurn() == 3 then
		if actorListCount > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList[R], 5180, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 4 then
		if protectListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, protectList1[T], 5183, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end	
	elseif battle:GetCurTurn() == 5 then
		if protectListCount2 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, protectList2[U], 5184, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end		
	elseif battle:GetCurTurn() == 6 then
		if protectListCount3 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, protectList3[V], 5185, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end	
	else
