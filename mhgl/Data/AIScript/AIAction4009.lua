function AIAction4009(battle, mob)	
	...
	for i = 0, actorCount - 1 do
		...
		--回龙克敌术目标选择列表
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and actor:GetFaction() == 3 and actor:GetCclass() == 3) then
		...
		--鬼门阵目标选择列表
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and actor:GetIndex() == mob:GetIndex() and not actor:IsBuffExist(1804)) then
	...
	if battle:GetCurTurn() == 1 then
		if actorListCount > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList[R], 5140, 0);	
		else
	...
	elseif battle:GetCurTurn() == 2 then
		if protectListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, protectList1[S], 5143, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
