function AIAction1002(battle, mob)
	...
	for i = 0, actorCount - 1 do
		...
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget()) then
			...
			if actor:GetHP() < actor:GetMaxHP() * 0.1 then
				AtkRate = 70;
				SkillRate = 70;
				DefRate = 70;
				ProtectRate = 100;
	...
	if actorListCount > 0 then
		...
		elseif Rate > SkillRate and Rate <= DefRate then --def 10% 0%
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		elseif Rate > DefRate and Rate <= ProtectRate then --protect 0% 30%
			if protectListCount > 0 then
				if protectActor ~= -1 then
					...
					mob:InputCommand(LuaI.BATCMD_SKILL, protectList[P], randomSkill, 0);
				end
