function AIAction0005(battle, mob)
	--保守型
	...
	if actorListCount > 0 then
		...
		elseif Rate > SkillRate and Rate <= DefRate then -- def 30%
			if protectListCount > 5 then
				local R = math.random(0, actorListCount - 1);
				if atkActor ~= -1 then
					mob:InputCommand(LuaI.BATCMD_ATTACK, atkActor, 0, 0);
				else
		...
		elseif Rate > DefRate and Rate <= ProtectRate then --protect 9%
