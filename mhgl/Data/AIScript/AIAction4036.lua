function AIAction4036(battle, mob)	
	--GUEST货商保镖的AI
	...
	for i = 0, actorCount - 1 do
		...
		--弛纵心猿秘法有效目标选择列表
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and not actor:IsBuffExist(1111)) then
	...
	elseif 	Rate > AtkRate and Rate <= SkillRate then
		if actorListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList1[S], 5165, 0);
	...
	elseif Rate > SkillRate and Rate <= ProtectRate then
		if protectListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_PROTECT, protectList1[U], 0, 0);
		elseif actorListCount > 0 then
