function AIAction1000(battle, mob)
	--BOSS级攻击AI
	...
	for i = 0, actorCount - 1 do
		...
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget()) then
			...
			if actor:GetLevel() > MaxLevel then
			   MaxLevel = actor:GetLevel()
			end
			if actor:GetLevel() < MinLevel then
			   MinLevel = actor:GetLevel()
			end
		end
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget()) then
	...
	if mob:GetMob_id() == 431 or mob:GetMob_id() == 579 then
	   if MaxLevel > 118 and MinLevel > 109 then
	      mob:SetLevel(120)
	   elseif MaxLevel > 118 and MinLevel > 100 then
	      mob:SetLevel(MaxLevel-5)
	   elseif MaxLevel > 118 and MinLevel > 80 then
	      mob:SetLevel(MaxLevel-10)
	   elseif MaxLevel > 118 and MinLevel < 80 then
	      mob:SetLevel(MaxLevel-10)
	   elseif MaxLevel > 110 and MinLevel > 100 then
	      mob:SetLevel(MaxLevel)
	   elseif MaxLevel > 110 and MinLevel > 90 then
	      mob:SetLevel(MaxLevel-5)
	   elseif MaxLevel > 110 and MinLevel > 80 then
	      mob:SetLevel(MaxLevel-10)
	   elseif MaxLevel > 110 and MinLevel > 60 then
	      mob:SetLevel(MaxLevel-10)
	   elseif MaxLevel > 110 and MinLevel <= 60 then
	      mob:SetLevel(MaxLevel-10)
	   elseif MaxLevel > 100 and MinLevel > 90 then
	      mob:SetLevel(50)
	   elseif MaxLevel > 100 and MinLevel < 90 then
	      mob:SetLevel(40)
	   elseif MaxLevel > 80 and MinLevel > 59 then
	      mob:SetLevel(30)
	   elseif MaxLevel > 1 and MinLevel > 1 then
	      mob:SetLevel(20)
	   end
	end
		
	local Rate = math.random(1, 100);
	local SkillRand = math.random(1, 10);
	...
	if actorListCount > 0 then
		...
		elseif Rate > AtkRate and Rate <= SkillRate then --skill 100%
			if mob:IsBuffExist(1103) or mob:IsBuffExist(1104) or mob:IsBuffExist(1105) or mob:IsBuffExist(1116) or mob:IsBuffExist(1809) then				
				...
				if mob:GetMob_id() == 431 or mob:GetMob_id() == 579 then
				   if turn > 20 and SkillRand < 5 then
				      randomSkill = 6202
				   elseif randomSkill == 6202 then
				      randomSkill = RandTable({5060,5081,5088})
				   end
				end
