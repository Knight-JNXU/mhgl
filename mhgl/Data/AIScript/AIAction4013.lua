function AIAction4013(battle, mob)	
	...
	for i = 0, actorCount - 1 do
		...
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and (not actor:IsBuffExist(1100) and not actor:IsBuffExist(1101) and
			...
			not actor:IsBuffExist(1112) and not actor:IsBuffExist(1113) and not actor:IsBuffExist(1114) and not actor:IsBuffExist(1115) and not actor:IsBuffExist(1116)) and (not actor:HasSkill(5339) and not actor:HasSkill(5330) and not actor:HasSkill(5263))) then
			actorList1[actorListCount1] = actor:GetIndex();
			actorListCount1 = actorListCount1 + 1;
		end
		
		--回龙克敌术目标选择列表
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and actor:GetFaction() == 2 and actor:GetCclass() == 1) then
		...
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and (not actor:IsBuffExist(1100) and not actor:IsBuffExist(1101) and
			not actor:IsBuffExist(1102) and not actor:IsBuffExist(1103) and not actor:IsBuffExist(1104) and not actor:IsBuffExist(1105) and not actor:IsBuffExist(1106) and
			not actor:IsBuffExist(1107) and not actor:IsBuffExist(1108) and not actor:IsBuffExist(1109) and not actor:IsBuffExist(1110) and not actor:IsBuffExist(1111) and
			not actor:IsBuffExist(1112) and not actor:IsBuffExist(1113) and not actor:IsBuffExist(1114) and not actor:IsBuffExist(1115) and not actor:IsBuffExist(1116)) and actor:HasSkill(5263)) then
		...
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and 
			actor:GetIndex() == mob:GetIndex() and actor:GetHP() <= actor:GetMaxHP() * 0.3) then
		...
		--回魂转念目标选择列表
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and 
			actor:GetIndex() == mob:GetIndex() and actor:GetSP() < actor:GetLevel()) then
	...
	if battle:GetCurTurn() == 1 then
		if actorListCount > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList[R], 5164, 0);	
		else
	...
	elseif battle:GetCurTurn() == 2 then
		if actorListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList1[S], 5160, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 3 then
		if actorListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList1[T], 5161, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 4 then
		if actorListCount3 > 0 then
			local U = math.random(0, actorListCount3 - 1);
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList3[U], 5162, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 5 then
		if actorListCount3 > 0 then
			local V = math.random(0, actorListCount3 - 1);
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList3[V], 5163, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 6 then
		if actorListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList1[W], 5165, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 7 then
		if protectListCount3 > 0 then
			local X = math.random(0, protectListCount3 - 1);
			mob:InputCommand(LuaI.BATCMD_SKILL, protectList3[X], 5293, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	else
