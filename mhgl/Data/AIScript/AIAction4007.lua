function AIAction4007(battle, mob)	
	...
	for i = 0, actorCount - 1 do
		...
		--回龙克敌术目标选择列表
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and actor:GetFaction() == 3 and actor:GetCclass() == 2) then
		...
		--罗刹魅目标选择列表
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and not actor:IsBuffExist(1503)) then
		...
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and 
			actor:GetIndex() == mob:GetIndex() and not actor:IsBuffExist(1701)) then
	...
	if battle:GetCurTurn() == 1 then
		if protectListCount2 > 0 then
			local T = math.random(0, protectListCount2 - 1);
			mob:InputCommand(LuaI.BATCMD_SKILL, protectList2[T], 5105, 0);	
		else
	...
	elseif battle:GetCurTurn() == 2 then
		if actorListCount > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList[R], 5100, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	...
	elseif battle:GetCurTurn() == 5 then
		if actorListCount > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList[U], 5102, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 6 then
		if actorListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList1[V], 5103, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	else
