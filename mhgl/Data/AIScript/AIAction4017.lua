function AIAction4017(battle, mob)	
	...
	for i = 0, actorCount - 1 do
		...
		--回龙克敌术目标选择列表
		if (actor ~= nil and actor:GetTeam() ~= mob:GetTeam() and actor:IsValidTarget() and actor:GetFaction() == 3 and actor:GetCclass() == 1) then
		...
		--云烟四起目标选择列表
		if (actor ~= nil and actor:GetTeam() == mob:GetTeam() and actor:IsValidTarget() and not actor:IsBuffExist(1803)) then
	...
	if battle:GetCurTurn() == 1 then
		if actorListCount > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList[R], 5120, 0);	
		else
	...
	elseif battle:GetCurTurn() == 2 then
		if actorListCount > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList[T], 5121, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 3 then
		if actorListCount1 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList1[S], 5122, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	elseif battle:GetCurTurn() == 4 then
		if actorListCount3 > 0 then
			...
			mob:InputCommand(LuaI.BATCMD_SKILL, actorList3[U], 5123, 0);	
		else
			mob:InputCommand(LuaI.BATCMD_DEFEND, 0, 0, 0);
		end
	else
